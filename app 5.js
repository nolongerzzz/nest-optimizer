...
function capFromEdges(edges, axis, plane, keepMin) {
  ...
  // FILLET FIX — corner blend, boundary stitch, self-intersection repair (see cut-edge-fillet-algorithm.md).
  ...
  //
  // v2: tries EIGHT directions around the point (not just ±perpendicular to
  // the longest edge), and tries all three vertices as the one to nudge, not
  // just the "middle" one. The v1 version had a real gap — if both
  // perpendicular directions happened to cross the matching-tolerance
  // boundary for that specific vertex, it silently gave up and left a
  // zero-area sliver in place (confirmed live on the cable-bar piece — the
  // creased-fan artifact at the corner/flat-run handoff). This version only
  // gives up if all three vertices fail in all eight directions at all five
  // magnitudes. ----
  function nudgeDegenerateTriangles(triangles2D) {
    ...
    for (const tri of triangles2D) {
      ...
      let fixed = false;
      for (let vIdx = 0; vIdx < 3 && !fixed; vIdx++) {
        const origPt = tri[vIdx].slice();
        const origKey = keyOf2(origPt);
        for (const [dx, dy] of dirs) {
          for (const mag of candidateMags) {
            const cand = [origPt[0] + dx * mag, origPt[1] + dy * mag];
            if (keyOf2(cand) !== origKey) continue;
            const testTri = tri.slice();
            testTri[vIdx] = cand;
            if (area2(testTri[0], testTri[1], testTri[2]) > 1e-9) {
              tri[vIdx] = cand;
              fixed = true;
              break;
            }
      ...
      // if still not fixed after all 3 vertices × 8 directions × 5
      // magnitudes: leave as-is. Now a rare last resort, not the routine
      // outcome for any vertex whose perpendicular happened to be unlucky.
    }
    return triangles2D;
  }
    ...
    function computeRingAtDepth(Rarr, s) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (const c of centers) {
          ...
          // BUG 4 FIX: the corner-sphere formula does not reduce back to the
          // original (unrounded) vertex position at t=0 — it's offset by an
          // amount unrelated to where the true boundary actually is.
          // Applying it unconditionally at every ring depth corrupts ring 0
          // (the body plane), which is exactly what the stitch connects to
          // the clipped body's true boundary. That corruption is the
          // fold/pinch at the corner-to-flat-run handoff (confirmed live).
          // Gating by `t` (0 at the body plane, 1 at the tip) makes the
          // corner blend fade in from exactly zero, matching the simple
          // per-vertex fillet's own behavior, so ring 0 stays untouched and
          // matches the true boundary exactly.
          const w = wWindow * t;
  ...
  loops.forEach(loop => {
    ...
    let used = false;
    if (Rreq >= 0.15) {
      const fil = filletLoop(loop, Rreq);
      if (fil && fil.length) {
