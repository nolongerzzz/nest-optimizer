...
function buildPlateMesh() {
  if (state.plateMesh) {
    if (state.plateMesh.userData.border) {
      state.scene.remove(state.plateMesh.userData.border);
    }
  ...
  state.plateMesh.userData.border = line;

  state.scene.add(state.plateMesh);

  if (state.controls) {
    state.controls.target.set(0, 0, 0);
    state.camera.position.set(p.w * 0.65, p.w * 0.75, p.d * 0.85);
...
function addModel(name, geometry) {
  ...
  setStatus(`Loaded: ${name} — hit Optimize to pack`);
}
...
function previewModelOnPlate(geometry, size) {
  ...
  // Clear previous preview / placed meshes only (keep state.placed if any)
  ...
  mesh.position.set(0, size.y / 2 + 0.2, 0);
  state.modelGroup.add(mesh);

  // Make sure plate is visible
  if (state.controls) {
    const p = getCurrentPlate();
    state.controls.target.set(0, 0, 0);
    state.camera.position.set(p.w * 0.65, p.w * 0.75, p.d * 0.85);
...
function runOptimize() {
  ...
  try {
    ...
    // Clear only the visual meshes — do NOT wipe state.placed
    while (state.modelGroup && state.modelGroup.children.length) {
      const child = state.modelGroup.children[0];
      state.modelGroup.remove(child);
      if (child.geometry) child.geometry.dispose();
      if (child.material) child.material.dispose();
    ...
    placed.forEach((p, idx) => {
      const mesh = new THREE.Mesh(p.geometry, material.clone());
