...
function paintSixOuter(m) {
  ...
  for (var i = 0; i < snap.length; i++) {
    var e = snap[i];
    e.d = e.n[0] * (e.keepMin ? lo[e.dispAxis] : hi[e.dispAxis]);
  }
...
function gradeStatus() {
  ...
  var armed = /click a face/i.test(t);
  var baked = /baked/i.test(t) && /painted/i.test(t);
  var fail = /fail/i.test(t);
  if (baked && !armed) return { ok: true, text: t };
  if (armed) return { ok: false, text: 'DRIVE FAIL — Soften armed click-a-face. ' + t };
  if (fail) return { ok: false, text: 'DRIVE FAIL — ' + t };
  return { ok: false, text: 'DRIVE MISS — status: ' + (t || '(empty)') };
}
