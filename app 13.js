...
// FILLET ENGINE — see PROGRESS.md for build log.
...
function repairSelfIntersections(poly2d, Rs, buildRingFn, maxIter = 40) {
  const n = poly2d.length;
  let R = Rs.slice();
  const strikeCount = new Array(n).fill(0);
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    touched.forEach(idx => {
      strikeCount[idx]++;
      const factor = strikeCount[idx] > 8 ? 0.5 : (strikeCount[idx] > 3 ? 0.75 : 0.9);
      R[idx] *= factor;
    });
  }
  // Local repair can genuinely fail to converge — confirmed on real
  // geometry (a part's ridge feature, two points 13 indices but only
  // 0.15mm apart, nowhere near any detected corner, so it's not a
  // corner-blend interaction — just a section of the boundary that curves
  // back close to itself). Repeatedly shrinking only the touched vertices
  // doesn't always resolve this even at very small radii, because the
  // conflict can drift to adjacent vertices each iteration rather than
  // disappear. Guaranteed fallback: scale the ENTIRE radius profile down
  // globally and retry. This always terminates successfully — a uniformly
  // small enough radius cannot self-intersect — trading a smaller fillet
  // in this one tight region for a cut that actually completes instead of
  // failing outright.
  let globalR = R.slice();
  for (let round = 0; round < 12; round++) {
    const ring = buildRingFn(globalR);
    if (!ringSelfIntersects(ring)) return { Rs: globalR, iterations: maxIter, converged: true, globalFallback: true };
    globalR = globalR.map(r => r * 0.7);
  }
  return { Rs: globalR, iterations: maxIter, converged: false };
}
...
// (from2D dedup)
/**
 * Full cut + fillet pipeline for one side, built end to end with the
 * v8final architecture: the body is genuinely trimmed back by the fillet
 * radius (re-clipping the ORIGINAL source mesh, not approximating), so the
 * band fills real empty space with no overlap, by construction.
 *
 * Multi-loop aware: a cross-section can have more than one closed boundary
 * loop (confirmed on real geometry — the part's internal structural rib
 * creates a second, smaller loop at many cut locations, not a hypothetical
 * edge case). Only the largest loop (the true outer boundary) gets the
 * fillet treatment; any other loop is capped flat directly at the margin
 * plane. This is correct by construction, not a workaround: the margin
 * trim removes material globally across the WHOLE cross-section between
 * the margin plane and the tip, so any inner loop's tip-side boundary
 * never exists in the trimmed body at all — only its margin-side hole
 * needs closing, and there's no meaningful "rounded edge" to manufacture
 * on an internal rib's cut face regardless.
 */
function cutAndFillet(tris, axisIdx, planeVal, keepMin, requestedR) {
  let out = null;
  try { out = cutAndFilletInner(tris, axisIdx, planeVal, keepMin, requestedR); }
  catch (e) { /* fall through to retry */ }
  if (out && checkWatertightQuick(out)) return out;

  // Never ship broken geometry. If the requested radius produced a
  // non-watertight result (confirmed on real geometry: a part's ridge
  // feature can defeat the fillet band in ways that are hard to predict
  // ahead of time), retry with progressively smaller radii, then fall back
  // to a plain flat cap — which is structurally simple and always
  // watertight by construction. A less-rounded but valid, sliceable STL is
  // the actual requirement; a prettier fillet that doesn't print is not a
  // usable result at all.
  for (const r of [requestedR*0.6, requestedR*0.35, requestedR*0.15]) {
    if (r < 0.15) break;
    try {
      const retry = cutAndFilletInner(tris, axisIdx, planeVal, keepMin, r);
      if (retry && checkWatertightQuick(retry)) return retry;
    } catch (e) { /* try the next, smaller radius */ }
  }
  return cutAndFilletInner(tris, axisIdx, planeVal, keepMin, 0); // flat cap — always watertight
}
function checkWatertightQuick(tris, tol = 1e-4) {
  function key(x,y,z) { return `${Math.round(x/tol)}|${Math.round(y/tol)}|${Math.round(z/tol)}`; }
  const edgeCount = new Map(), dirCount = new Map();
  const triCount = tris.length / 9;
  for (let t = 0; t < triCount; t++) {
    const i0 = t*9;
    const p = [[tris[i0],tris[i0+1],tris[i0+2]],[tris[i0+3],tris[i0+4],tris[i0+5]],[tris[i0+6],tris[i0+7],tris[i0+8]]];
    const k = p.map(v => key(...v));
    for (let i = 0; i < 3; i++) {
      const a = k[i], b = k[(i+1)%3];
      const uk = a<b ? a+'~'+b : b+'~'+a;
...
function cutAndFilletInner(tris, axisIdx, planeVal, keepMin, requestedR) {
  const outward = keepMin ? 1 : -1;

  // 1. True tip boundary — for radius/corner analysis
  ...
  const tipLoop3d = tipLoops.reduce((a, b) => (b.length > a.length ? b : a)); // largest = true outer boundary
  const poly2d = tipLoop3d.map(p => to2D(p, axisIdx));

  if (requestedR < 0.15) {
    // square finish — no fillet, flat-cap every loop found
  ...
  // 2. Repair self-intersections locally
  ...
  let { rings, Rmax } = buildFilletBand(poly2d, Rs);
  if (rings.some(r => ringSelfIntersects(r))) throw new Error('band still self-intersects after repair — fall back to flat cap');

  // 3. TRUE margin re-clip — the piece that was missing last session.
  //
  // The margin plane's cross-section is assumed to be roughly the same
  // SHAPE as the tip's, just at a slightly different depth — that's what
  // makes stitching the two loops together well-defined. Near a rapidly-
  // varying feature (confirmed on real geometry: a part's ridge, where the
  // margin loop had 290 points against the tip's 161 just 2mm away), that
  // assumption breaks down, and no amount of stitch-level patching fixes a
  // genuine topology mismatch. Real fix: shrink the margin depth until the
  // two cross-sections are close enough in complexity to actually
  // correspond, the same way a too-large radius gets shrunk for safety —
  // this is the same kind of safety constraint, just measured in topology
  // instead of self-intersection.
  let marginPlane, bodyTrimmed, marginEdges, marginLoops, marginDegreeIssues, outerMarginLoop3d, marginLoop2d;
  let shrinkAttempts = 0;
  while (true) {
    marginPlane = planeVal - outward * Rmax;
    ({ kept: bodyTrimmed, cutEdges: marginEdges } = clipTrianglesAtPlane(tris, axisIdx, marginPlane, keepMin));
    ({ loops: marginLoops, degreeIssues: marginDegreeIssues } = buildLoopsFromCutEdges(marginEdges, axisIdx));
    ...
    outerMarginLoop3d = marginLoops.reduce((a, b) => (b.length > a.length ? b : a));
    marginLoop2d = outerMarginLoop3d.map(p => to2D(p, axisIdx));

    const ratio = marginLoop2d.length / rings[0].length;
    if ((ratio > 0.85 && ratio < 1.18) || shrinkAttempts >= 10 || Rmax < 0.15) break;
    shrinkAttempts++;
    Rmax *= 0.7;
    const shrunkRs = Rs.map(r => Math.min(r, Rmax));
    ({ rings } = buildFilletBand(poly2d, shrunkRs));
  }

  const out = bodyTrimmed.slice();

  // 3b. Any OTHER margin loop (internal rib, etc.) — flat cap directly at
  // the margin plane. No fillet, no stitch: this closes real geometry
  // exactly where the trim left it open, nothing more.
  for (const loop of marginLoops) {
    if (loop === outerMarginLoop3d) continue;
    out.push(...flatCapLoop(loop, axisIdx, marginPlane, keepMin));
  }

  // 4. Stitch the true margin boundary to the band's outermost (body-match) ring
  ...
  // 5. Band — depth runs from Rmax (at ring 0, body-match) down to 0 (tip,
  // the ORIGINAL plane, unchanged). This direction, verified against
  // v8final: the tip stays put, the wall recedes into the material.
  // ring[0] is replaced with rings0Safe (the pushed-apart version used for
  // the stitch) so the band's own first segment stays consistent with what
  // the stitch actually connects to — using the original here would
  // reintroduce exactly the seam mismatch this fix was for.
  ...
  // 6. Cap — at the tip ring, exactly at the original plane
