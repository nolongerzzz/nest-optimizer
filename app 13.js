...
function undoLast() {
  ...
  if (entry.type === 'splitReplace') {
    ...
    if (entry.cutT != null) state.cutT = entry.cutT;
    // Plate still shows the two halves — wipe them
    clearPlaced();
    clearDisplayMeshes();
    removeCutHelper();
    ...
    if (src && src.geometry) {
      previewModelOnPlate(src.geometry, src.size);
    }
    setStatus('Undo: restored single piece before split');
    return;
  }
...
function cutActiveModel() {
  ...
  setStatus(
    ...
    ' mm on plate. Original replaced (1→2).' +
