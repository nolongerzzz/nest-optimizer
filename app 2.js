  ...
  if (state.joinSession && state.modelGroup) {
    ...
    if (jHits.length) {
      ...
      if (typeof idx === 'number' && state.placed[idx] && state.placed[idx].sourceId != null) {
        ...
        } else if (sid === state.editId || sid === state.joinPartnerId) {
          if (sid === state.editId) state.joinFaceA = planar;
          else state.joinFaceB = planar;
          if (planar) showPlanarHighlight(jHits[0].object, planar);
          updateJoinUI();
        }
        event.stopPropagation();
...
function joinHalvesOnPlate(modelA, placedA, modelB, placedB, axisIdx) {
  ...
  merged = capSoupNearPlane(merged, axisIdx, planeL, 0.8);
  merged = flattenOuterWalls(merged, 0.28);
  merged = capAllOuterHoles(merged);
  merged = weldSoupVerts(merged, 0.35);
...
function repairJoinedSoup(soup) {
  try {
    ...
    // 5) Flip inverted triangles so normals point outward from the bbox center.
    const bb = bboxOf(soupC);
    const cx0 = (bb.minX + bb.maxX) / 2;
    const cy0 = (bb.minY + bb.maxY) / 2;
    const cz0 = (bb.minZ + bb.maxZ) / 2;
    const triCount3 = soupC.length / 9;
    const flipped = new Float32Array(soupC.length);
    for (let t = 0; t < triCount3; t++) {
      const i0 = t * 9;
      const ax = soupC[i0], ay = soupC[i0 + 1], az = soupC[i0 + 2];
      const bx = soupC[i0 + 3], by = soupC[i0 + 4], bz = soupC[i0 + 5];
      const cx = soupC[i0 + 6], cy = soupC[i0 + 7], cz = soupC[i0 + 8];
      ...
      const ny = uz * vx - ux * vz;
      const nz = ux * vy - uy * vx;
      const mx = (ax + bx + cx) / 3 - cx0;
      const my = (ay + by + cy) / 3 - cy0;
      const mz = (az + bz + cz) / 3 - cz0;
      const dot = nx * mx + ny * my + nz * mz;
      if (dot < 0) {
        flipped[i0] = ax; flipped[i0 + 1] = ay; flipped[i0 + 2] = az;
        flipped[i0 + 3] = cx; flipped[i0 + 4] = cy; flipped[i0 + 5] = cz;
        flipped[i0 + 6] = bx; flipped[i0 + 7] = by; flipped[i0 + 8] = bz;
      } else {
        flipped[i0] = ax; flipped[i0 + 1] = ay; flipped[i0 + 2] = az;
        flipped[i0 + 3] = bx; flipped[i0 + 4] = by; flipped[i0 + 5] = bz;
        flipped[i0 + 6] = cx; flipped[i0 + 7] = cy; flipped[i0 + 8] = cz;
      }
    }

    // 6) Safety net: never emit an empty mesh or a heavily shrunk bbox.
    if (flipped.length < 9) return original;
    const volAfter = bboxVolume(bboxOf(flipped));
...
function flattenOuterWalls(soup, tol) {
  if (!soup || soup.length < 9) return soup;
  let minX = Infinity, minY = Infinity, minZ = Infinity;
  let maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
