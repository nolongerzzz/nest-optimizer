...
function capFromEdges(edges, axis, plane, keepMin) {
  ...
  function filletLoop(loop3, Rreq) {
    ...
    for (let i = 0; i < n; i++) {
      let m = Rloc[i];
      for (let k = -2; k <= 2; k++) m = Math.min(m, Rloc[(i + k + n) % n]);
      Rs[i] = m;
    }
    ...
    for (let s = 0; s <= STEPS; s++) {
      const t = s / STEPS; // 0 at wall (u=R), 1 at cap (u=0)
      const ring = [];
      for (let i = 0; i < n; i++) {
        const R = Rs[i];
        const u = R * (1 - t); // R → 0
        ...
        for (const c of centers) {
          if (!c.C) continue;
          const dist = Math.min((i - c.i + n) % n, (c.i - i + n) % n);
          if (dist > windowN) continue;
          const w = smoothstep(1 - dist / windowN);
    ...
    for (let s = 0; s < rings.length - 1; s++) {
      ...
      const ua = Rmax * (1 - s / STEPS);
      const ub = Rmax * (1 - (s + 1) / STEPS);
      const alongA = plane - outward * (Rmax - ua);
      const alongB = plane - outward * (Rmax - ub);
      // wall at u=R sits at plane; cap at u=0 sits at plane - outward*R
      for (let i = 0; i < n; i++) {
        const i1 = (i + 1) % n;
        const A0 = from2At(a[i][0], a[i][1], plane - outward * (s / STEPS) * Rmax);
        const A1 = from2At(a[i1][0], a[i1][1], plane - outward * (s / STEPS) * Rmax);
        const B0 = from2At(b[i][0], b[i][1], plane - outward * ((s + 1) / STEPS) * Rmax);
        const B1 = from2At(b[i1][0], b[i1][1], plane - outward * ((s + 1) / STEPS) * Rmax);
        tris3.push([A0, A1, B1], [A0, B1, B0]);
      }
    }
    const inner = rings[rings.length - 1].map(uv => from2At(uv[0], uv[1], plane - outward * Rmax));
