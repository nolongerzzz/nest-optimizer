    ...
    if (modelHits.length) {
      ...
      if (typeof idx === 'number' && state.placed[idx]) {
        ...
        if (p.sourceId != null) {
          setOrbitFromPlate(false);
          if (state.joinSession && state.joinArmed) {
            assignJoinClick(p.sourceId, captureJoinFace(modelHits[0]));
            event.stopPropagation();
            return;
          }
    ...
    if (modelHits.length) {
      ...
      if (typeof idx === 'number' && state.placed.length) {
        ...
        if (state.joinSession && state.joinArmed && hit && hit.sourceId != null) {
          assignJoinClick(hit.sourceId, captureJoinFace(modelHits[0]));
          event.stopPropagation();
          return;
        }
...
function joinHalvesOnPlate(modelA, placedA, modelB, placedB, axisIdx) {
  ...
  let leftKept = stripFacingInBand(left, axisIdx, planeL, 1.6).tris;
  let rightKept = stripFacingInBand(right, axisIdx, planeR, 1.6).tris;
  const extraL = rawJoinStripCapAtPlane(leftKept, axisIdx, planeL, 1.2);
  const extraR = rawJoinStripCapAtPlane(rightKept, axisIdx, planeR, 1.2);
  ...
  merged = stripFacingInBand(merged, axisIdx, planeL, 1.8).tris;
...
function captureJoinFace(hit) {
  ...
  const sign = axis === 'x' ? (n.x >= 0 ? 1 : -1) : (n.z >= 0 ? 1 : -1);
  return {
    axis: axis,
    axisIdx: axis === 'x' ? 0 : 2,
    sign: sign,
...
function autoKissOnFaces(placedA, faceA, placedB, faceB) {
  ...
  const axis = (faceA && faceA.axis) || (faceB && faceB.axis) || detectMateAxis(placedA, placedB);
