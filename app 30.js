...
function undoLast() {
  ...
  if (entry.type === 'splitReplace') {
    ...
    updateCutterUI();
    updateEditSize();
    if (state.models.length >= 1) showAllModelsOnPlate();
    updateAdjustUI();
    updateUndoBtn();
...
function layoutAfterSplit(sourcePose, poseById, sourceId, modelA, modelB, axis) {
  ...
  const xA = sourcePose.x - (wB + gap) / 2;
  const xB = sourcePose.x + (wA + gap) / 2;
  if (modelA) placeModelMovable(modelA, xA, sourcePose.z);
  if (modelB) placeModelMovable(modelB, xB, sourcePose.z);
