...
/** Plate-space weld: strip each half's facing cap, slide kerf shut, merge. */
function joinHalvesOnPlate(modelA, placedA, modelB, placedB) {
  ...
  let sL = 0, sR = 0, leftKept, rightKept;
  const tols = [0.4, 0.8, 1.2, 2.0, 3.0];
  for (let i = 0; i < tols.length; i++) {
    const a = rawJoinStripCapAtPlane(left, 0, bbL.maxV, tols[i]);
    const b = rawJoinStripCapAtPlane(right, 0, bbR.minV, tols[i]);
    leftKept = a.tris; rightKept = b.tris; sL = a.stripped; sR = b.stripped;
    if (sL > 0 && sR > 0) break;
  }
  if (sL === 0 || sR === 0) {
    throw new Error('no facing caps (stripped ' + sL + '+' + sR + ')');
  }
  const closed = rawJoinTranslateAxis(rightKept, 0, bbL.maxV - bbR.minV);
  const merged = new Float32Array(leftKept.length + closed.length);
