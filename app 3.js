...
function clipGeometrySide(geometry, axis, plane, keepMin) {
  // Robust triangle–plane clip: walk each edge, keep polygon on one side, fan-triangulate.
  ...
  function keep(v) {
    return keepMin ? v <= plane + EPS : v >= plane - EPS;
  }
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    const ks = cs.map(keep);
    const nKeep = (ks[0] ? 1 : 0) + (ks[1] ? 1 : 0) + (ks[2] ? 1 : 0);
    if (nKeep === 0) continue;
    if (nKeep === 3) {
    ...
    // Build ordered polygon on the keep side by walking edges 0→1→2→0
    const poly = [];
    const cutPts = [];
    for (let e = 0; e < 3; e++) {
      const a = verts[e];
    ...
    // Fan triangulate polygon (preserves winding of original walk)
    for (let i = 1; i < clean.length - 1; i++) {
      pushTri(clean[0], clean[i], clean[i + 1]);
    }

    // Cap edge: the two intersection points on the plane (if any)
    if (cutPts.length >= 2) {
      // Use first and last unique cut points from edge walk
      let a = cutPts[0];
      let b = cutPts[cutPts.length - 1];
      for (let i = 1; i < cutPts.length; i++) {
        if (!almostSame(cutPts[0], cutPts[i])) {
          b = cutPts[i];
          break;
        }
      }
...
function cutActiveModel() {
  ...
  // Activate side A for next cut; reset plane to mid of new piece
  state.cutT = 0.5;
  if (state.cutterOpen) showEditPreview();
  setStatus('Split at ' + cutMm.toFixed(1) + ' mm on ' + axis.toUpperCase() + ' → ' + nameA + ' + ' + nameB + ' (original kept). Select a half to cut again.');
}
