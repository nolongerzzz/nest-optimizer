...
/** Cut plane in geometry local space — prefers visible red helper so cut == what you see */
function getCutPlaneLocal(model) {
  ...
  if (state.cutHelper && state.cutterOpen) {
    const plane = axis === 'x' ? state.cutHelper.position.x : state.cutHelper.position.z;
    const origin = axis === 'x' ? bbox.min.x : bbox.min.z;
    state.cutT = Math.min(0.98, Math.max(0.02, (plane - origin) / span));
    return { axis, plane, bbox, span };
  }

  const t = state.cutT;
  const plane = (axis === 'x' ? bbox.min.x : bbox.min.z) + t * span;
  return { axis, plane, bbox, span };
}
...
function cutActiveModel() {
  ...
  // Stay on the ORIGINAL piece so the view does not "shrink" to a half
  ...
  setStatus(
    ...
    'Original kept. Click a half in the list to edit it.' +
