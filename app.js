...
function layoutAfterSplit(sourcePose, poseById, sourceId, modelA, modelB, axis) {
  ...
  // A = keepMin = material left of the blade. B = keepMax = right of blade.
  // Old code parked B left / A right, which swapped halves after every cut.
  const wA = (modelA && modelA.size) ? modelA.size.x : 20;
  const wB = (modelB && modelB.size) ? modelB.size.x : 20;
  const xLeft = sourcePose.x - gap / 2 - wA / 2;
  const xRight = sourcePose.x + gap / 2 + wB / 2;
  if (modelA) placeModelMovable(modelA, xLeft, sourcePose.z);
  if (modelB) placeModelMovable(modelB, xRight, sourcePose.z);
