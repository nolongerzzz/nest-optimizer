...
function layoutAfterSplit(sourcePose, poseById, sourceId, modelA, modelB, axis, stayA, stayB) {
  ...
  const ax = sourcePose.x + (stayA && typeof stayA.x === 'number' ? stayA.x : 0);
  const az = sourcePose.z + (stayA && typeof stayA.z === 'number' ? stayA.z : 0);
  const bx = sourcePose.x + (stayB && typeof stayB.x === 'number' ? stayB.x : 0);
  const bz = sourcePose.z + (stayB && typeof stayB.z === 'number' ? stayB.z : 0);
...
function stripXFacingInBand(soup, plane, band) {
  const out = [];
...
/** Plate-space weld: strip facing X-caps, close kerf, weld verts, merge. */
function joinHalvesOnPlate(modelA, placedA, modelB, placedB) {
  ...
  const aIsLeft = placedA.x <= placedB.x;
  const left = aIsLeft ? soupA : soupB;
  const right = aIsLeft ? soupB : soupA;
  const bbL = rawJoinBboxOf(left, 0);
  const bbR = rawJoinBboxOf(right, 0);
  const planeL = bbL.maxV;
  const planeR = bbR.minV;
  let leftKept = stripXFacingInBand(left, planeL, 1.6).tris;
  let rightKept = stripXFacingInBand(right, planeR, 1.6).tris;
  const extraL = rawJoinStripCapAtPlane(leftKept, 0, planeL, 1.2);
  const extraR = rawJoinStripCapAtPlane(rightKept, 0, planeR, 1.2);
  ...
  const closed = rawJoinTranslateAxis(rightKept, 0, planeL - planeR);
  let merged = new Float32Array(leftKept.length + closed.length);
  merged.set(leftKept, 0);
  merged.set(closed, leftKept.length);
  merged = stripXFacingInBand(merged, planeL, 1.8).tris;
  merged = weldSoupVerts(merged, 0.18);
...
function joinSelectedModels() {
  ...
  try {
    if (placedA && placedB) {
      newGeo = joinHalvesOnPlate(modelA, placedA, modelB, placedB);
    } else {
