...
function buildPlateMesh() {
  ...
  state.plateMesh.add(line);

  state.scene.add(state.plateMesh);

  state.controls.target.set(0, 0, 0);
  state.camera.position.set(p.w * 0.6, p.w * 0.7, p.d * 0.8);
...
function runOptimize() {
  ...
  try {
    ...
    if (placed.length < instances.length) {
      ...
      setStatus(`Packed ${placed.length} models`, false);
    }
  } catch (err) {
    console.error(err);
...
function geometryToSTL(geometry) {
  ...
  let stl = 'solid nestoptimizer\n';
  for (let i = 0; i < pos.count; i += 3) {
    stl += `  facet normal 0 0 0\n    outer loop\n`;
    stl += `      vertex ${pos.getX(i).toFixed(4)} ${pos.getY(i).toFixed(4)} ${pos.getZ(i).toFixed(4)}\n`;
    stl += `      vertex ${pos.getX(i+1).toFixed(4)} ${pos.getY(i+1).toFixed(4)} ${pos.getZ(i+1).toFixed(4)}\n`;
    stl += `      vertex ${pos.getX(i+2).toFixed(4)} ${pos.getY(i+2).toFixed(4)} ${pos.getZ(i+2).toFixed(4)}\n`;
    stl += `    endloop\n  endfacet\n`;
  }
  stl += 'endsolid nestoptimizer\n';
  return stl;
}
...
function exportSTLs() {
  ...
  a.href = url;
  a.download = `nest-${getCurrentPlate().name.replace(/\s+/g, '_')}.stl`;
  a.click();
