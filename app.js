...
function rawFilletCut(rawTris, axisIdx, plane, keepMin, requestedR) {
  ...
  const marginAxisSpan = function (flat) {
    if (!flat || flat.length < 3) return -Infinity;
    let lo = Infinity, hi = -Infinity;
    for (let i = axisIdx; i < flat.length; i += 3) {
      const v = flat[i];
      if (v < lo) lo = v;
      if (v > hi) hi = v;
    }
  ...
  const clipA = rawClipTrianglesAtPlane(rawTris, axisIdx, marginPlane, keepMin);
  const clipB = rawClipTrianglesAtPlane(rawTris, axisIdx, marginPlane, !keepMin);
  const marginClip = marginAxisSpan(clipB.kept) > marginAxisSpan(clipA.kept) ? clipB : clipA;
  const { kept: bodyTrimmed, cutEdges: marginEdges } = marginClip;
...
function rawChamferCut(rawTris, axisIdx, plane, keepMin, R) {
  ...
  let poly2d = tipLoop3d.map(p => [p[other[0]], p[other[1]]]);
  poly2d = raw2DWeldLoop(poly2d, 0.08);
  if (poly2d.length < 8) throw new Error('boundary too small to chamfer after weld');
  ...
  const marginLoop2dTagged = marginLoop2d.map(p => [p[0], p[1], marginPlane]);
