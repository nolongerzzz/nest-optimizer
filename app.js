...
var NSO_CSG = (function () {
  ...
  // region it's safe to cut into: from the bit's mouth plane (frame.capCoord)
  // inward along the punch axis, stopping `minWall` short of hull A's own
  // local-frame extent on the far side. Exact for a flat/box hull; a
  // conservative (not exact) bound for a curved one -- see caveats.
  // Returns a world-space Manifold, or null if there's no room to cut.
  function buildSafetyBox(wasm, frame, minWall) {
    var axis = frame.axis, sign = frame.sign, cap = frame.capCoord;
    var box = frame.box;
    var axB = (axis + 1) % 3, axC = (axis + 2) % 3;

    var farLocal = (sign > 0)
      ? Math.max(cap, box.max.getComponent(axis) - minWall)
      : Math.min(cap, box.min.getComponent(axis) + minWall);

    var lo = new THREE.Vector3(), hi = new THREE.Vector3();
    var lateralPad = Math.max(1, frame.span * 0.05);
    ...
    var mouthPad = Math.max(1, frame.span * 0.05);
    if (sign > 0) {
      lo.setComponent(axis, cap - mouthPad);
      hi.setComponent(axis, farLocal);
    } else {
      lo.setComponent(axis, farLocal);
      hi.setComponent(axis, cap + mouthPad);
    }

    var size = new THREE.Vector3().subVectors(hi, lo);
    if (size.getComponent(axis) <= 1e-6) return null;
...
async function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  try {
    manA = NSO_CSG.soupToManifold(wasm, aWorld);
    if (manA.status() !== 'NoError') throw new Error('hull soup rejected by kernel: ' + manA.status());

    manB = NSO_CSG.soupToManifold(wasm, bWorld);
    if (manB.status() !== 'NoError') throw new Error('bit soup rejected by kernel: ' + manB.status());

    safetyBox = NSO_CSG.buildSafetyBox(wasm, frame, minWall);
    if (!safetyBox) throw new Error('no room to cut within safe wall margin');
