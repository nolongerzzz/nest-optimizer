...
function alignJoinForSlide() {
  ...
  if (Math.abs(dx) >= Math.abs(dz)) {
    const nx = dx >= 0
      ? placedB.x + (fa.maxx - fb.minx)
      : placedB.x + (fa.minx - fb.maxx);
    applyPlacedXZ(placedB, nx, placedB.z);
    state.joinSlideAxis = 'z';
  } else {
    const nz = dz >= 0
      ? placedB.z + (fa.maxz - fb.minz)
      : placedB.z + (fa.minz - fb.maxz);
    applyPlacedXZ(placedB, placedB.x, nz);
    state.joinSlideAxis = 'x';
  }
  setStatus('Edges aligned — B sits on A’s facing wall');
  return true;
}
