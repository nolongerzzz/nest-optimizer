...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  var canBox = !!(opts.plugMatrix && opts.plugLocalBox);
  if (mode === 'auto') mode = canBox ? 'box' : 'centroid';
  if (mode === 'box' && !canBox) mode = 'centroid';

  var out = [];
  var v = new THREE.Vector3();

  if (mode === 'box') {
    /* Work in the plug's local frame: the OBB becomes a true AABB there. */
    var toLocal = new THREE.Matrix4().copy(opts.plugMatrix).invert();
    var toWorld = opts.plugMatrix;
    var box = opts.plugLocalBox;

    var a = new THREE.Vector3(), b = new THREE.Vector3(), c = new THREE.Vector3();
    var frag = [];

    for (var i = 0; i < aWorld.length; i += 9) {
      a.set(aWorld[i], aWorld[i + 1], aWorld[i + 2]).applyMatrix4(toLocal);
      b.set(aWorld[i + 3], aWorld[i + 4], aWorld[i + 5]).applyMatrix4(toLocal);
      c.set(aWorld[i + 6], aWorld[i + 7], aWorld[i + 8]).applyMatrix4(toLocal);

      // fast accept: triangle AABB misses the box entirely
      var mnx = Math.min(a.x, b.x, c.x), mxx = Math.max(a.x, b.x, c.x);
      var mny = Math.min(a.y, b.y, c.y), mxy = Math.max(a.y, b.y, c.y);
      var mnz = Math.min(a.z, b.z, c.z), mxz = Math.max(a.z, b.z, c.z);
      if (mxx < box.min.x || mnx > box.max.x ||
          mxy < box.min.y || mny > box.max.y ||
          mxz < box.min.z || mnz > box.max.z) {
        out.push(aWorld[i], aWorld[i + 1], aWorld[i + 2],
                 aWorld[i + 3], aWorld[i + 4], aWorld[i + 5],
                 aWorld[i + 6], aWorld[i + 7], aWorld[i + 8]);
        continue;
      }

      frag.length = 0;
      NSO_clipTriOutsideAABB(a, b, c, box, frag);
      for (var f = 0; f < frag.length; f += 3) {
        v.set(frag[f], frag[f + 1], frag[f + 2]).applyMatrix4(toWorld);
        out.push(v.x, v.y, v.z);
      }
    }
  } else {
    /* degrade: centroid filter, no invented CSG */
    for (var i2 = 0; i2 < aWorld.length; i2 += 9) {
      var cxx = (aWorld[i2] + aWorld[i2 + 3] + aWorld[i2 + 6]) / 3;
      var cyy = (aWorld[i2 + 1] + aWorld[i2 + 4] + aWorld[i2 + 7]) / 3;
      var czz = (aWorld[i2 + 2] + aWorld[i2 + 5] + aWorld[i2 + 8]) / 3;
      if (!NSO_pointInsideSoup(bWorld, cxx, cyy, czz)) {
        out.push(aWorld[i2], aWorld[i2 + 1], aWorld[i2 + 2],
                 aWorld[i2 + 3], aWorld[i2 + 4], aWorld[i2 + 5],
                 aWorld[i2 + 6], aWorld[i2 + 7], aWorld[i2 + 8]);
      }
    }
  }
  ...
  /* Cavity walls: plug faces inside the hull, wound inward. Pocket stays OPEN. */
  var wall = 0;
  for (var j = 0; j < bWorld.length; j += 9) {
    var bx = (bWorld[j] + bWorld[j + 3] + bWorld[j + 6]) / 3;
    var by = (bWorld[j + 1] + bWorld[j + 4] + bWorld[j + 7]) / 3;
    var bz = (bWorld[j + 2] + bWorld[j + 5] + bWorld[j + 8]) / 3;
    if (!NSO_pointInsideSoup(aWorld, bx, by, bz)) continue;
    // reversed winding -> normals face into the pocket void
    out.push(bWorld[j], bWorld[j + 1], bWorld[j + 2],
             bWorld[j + 6], bWorld[j + 7], bWorld[j + 8],
             bWorld[j + 3], bWorld[j + 4], bWorld[j + 5]);
    wall++;
  }

  /* fail safe */
  if (hullKept === 0) return { ok: false, soup: aWorld, mode: mode, reason: 'all hull tris removed' };
  if (hullKept >= na && wall === 0) return { ok: false, soup: aWorld, mode: mode, reason: 'plug missed hull' };
  if (wall === 0) return { ok: false, soup: aWorld, mode: mode, reason: 'no plug faces inside hull' };
...
/* ============================================================
   3. subtractBFromA — mesh-level wrapper
   ============================================================ */

function NSO_subtractMeshes(meshA, meshB, opts) {
  opts = opts || {};
  if (!meshA || !meshB) return { ok: false, reason: 'missing mesh' };

  var aWorld = meshToWorldSoup(meshA, opts.liftA);
  var bWorld = meshToWorldSoup(meshB, opts.liftB);

  meshB.updateWorldMatrix(true, false);
  var localBox = NSO_localAABB(meshB.geometry);
  var boxOk = (opts.mode === 'box') || NSO_isBoxLike(meshB.geometry, localBox);

  var res = subtractSoupBFromA(aWorld, bWorld, {
    plugMatrix: boxOk ? meshB.matrixWorld.clone() : null,
    plugLocalBox: boxOk ? localBox : null,
    mode: opts.mode || 'auto'
  });

  if (!res.ok) {
    console.warn('[NSO] subtract skipped, A untouched:', res.reason);
    return res;
  }

  var localSoup = NSO_soupToLocal(res.soup, meshA);
  var newGeom = NSO_soupToGeometry(localSoup);

  if (typeof subtractReplace === 'function') {
    subtractReplace(meshA, newGeom);
  } else {
    var old = meshA.geometry;
    meshA.geometry = newGeom;
    if (old && old.dispose) old.dispose();
  }

  console.log('[NSO] subtract mode=' + res.mode +
              ' hull=' + res.hullTris + ' walls=' + res.wallTris);
  return res;
}

/* ============================================================
   4. seatFlushBitToHull — outer face on the skin, body inside
   ============================================================
   opts: { proud: 0 }  ( >0 = bit stands slightly proud, <0 = slight bury )
   ============================================================ */
...
function NSO_seatFlushBitToHull(hullMesh, bitMesh, opts) {
  opts = opts || {};
  var proud = (opts.proud === undefined) ? 0 : opts.proud;
  if (!hullMesh || !bitMesh) return { ok: false, reason: 'missing mesh' };

  var hull = meshToWorldSoup(hullMesh, opts.liftHull);
  var bit = meshToWorldSoup(bitMesh, opts.liftBit);
  if (!hull.length || !bit.length) return { ok: false, reason: 'empty soup' };

  /* bit centroid, world */
  var bc = new THREE.Vector3();
  for (var i = 0; i < bit.length; i += 3) bc.x += bit[i], bc.y += bit[i + 1], bc.z += bit[i + 2];
  bc.multiplyScalar(3 / bit.length);

  /* hull centroid, for outward orientation */
  var hc = new THREE.Vector3();
  for (var h = 0; h < hull.length; h += 3) hc.x += hull[h], hc.y += hull[h + 1], hc.z += hull[h + 2];
  hc.multiplyScalar(3 / hull.length);

  /* nearest hull triangle to the bit centroid */
  var best = Infinity, P = new THREE.Vector3(), N = new THREE.Vector3();
  var a = new THREE.Vector3(), b = new THREE.Vector3(), c = new THREE.Vector3();
  var q = new THREE.Vector3(), nn = new THREE.Vector3();
  var e1 = new THREE.Vector3(), e2 = new THREE.Vector3();

  for (var t = 0; t < hull.length; t += 9) {
    a.set(hull[t], hull[t + 1], hull[t + 2]);
    b.set(hull[t + 3], hull[t + 4], hull[t + 5]);
    c.set(hull[t + 6], hull[t + 7], hull[t + 8]);
    NSO_closestPointOnTri(bc, a, b, c, q);
    var d = q.distanceToSquared(bc);
    if (d < best) {
      best = d;
      P.copy(q);
      e1.subVectors(b, a); e2.subVectors(c, a);
      nn.crossVectors(e1, e2);
      if (nn.lengthSq() > 1e-16) N.copy(nn).normalize();
    }
  }
  if (best === Infinity) return { ok: false, reason: 'no hull triangle' };

  /* force N outward */
  if (N.dot(new THREE.Vector3().subVectors(P, hc)) < 0) N.negate();

  /* flush: bit's OUTERMOST extent along N lands on the skin, not 70% buried */
  var maxProj = -Infinity;
  for (var v = 0; v < bit.length; v += 3) {
    var pr = bit[v] * N.x + bit[v + 1] * N.y + bit[v + 2] * N.z;
    if (pr > maxProj) maxProj = pr;
  }
  var target = P.dot(N) + proud;
  var delta = target - maxProj;

  var offset = N.clone().multiplyScalar(delta);
  bitMesh.position.add(offset);
  ...
  return {
    ...
    normal: N.clone(),
    contact: P.clone(),
    offset: offset,          // negate + re-add for undo
    delta: delta
  };
}
...
function subtractBFromA() {
  ...
  try {
    ...
    const localBox = NSO_localAABB(placedB.mesh.geometry);
    const res = subtractSoupBFromA(aWorld, bWorld, {
      plugMatrix: placedB.mesh.matrixWorld.clone(),
      plugLocalBox: localBox,
      mode: 'box'
    });
