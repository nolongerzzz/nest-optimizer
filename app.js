...
function meshBandExtent(mesh, axis, tMin, tMax, pad) {
  const out = { min: Infinity, max: -Infinity, hits: false, walls: [] };
  ...
  for (let ti = 0; ti < triCount; ti++) {
    ...
    // staying flat) -- these aren't real walls and pollute min/max.
    if (nMax - nMin > WALL_FLATNESS_TOL) continue;
    out.hits = true;
    if (nMin < out.min) out.min = nMin;
    if (nMax > out.max) out.max = nMax;
    const wv = (nMin + nMax) / 2;
    let dup = false;
    for (let w = 0; w < walls.length; w++) {
      if (Math.abs(walls[w] - wv) < 0.15) { dup = true; break; }
    }
    if (!dup) walls.push(wv);
  }
  out.walls = walls;
  return out;
}
...
function alignJoinForSlide() {
  ...
  function nearestWall(walls, target) {
    let best = null, bestD = Infinity;
    for (let i = 0; i < walls.length; i++) {
      const d = Math.abs(walls[i] - target);
      if (d < bestD) { bestD = d; best = walls[i]; }
    }
    return best;
  }
  ...
  // Candidates now pick the A-wall NEAREST to B's corresponding edge, not
  // the global extreme across the whole band -- an L/step-shaped A can have
  // several distinct wall faces (e.g. an outer wall and an inner notch wall)
  // inside the same band, and the old min/max collapse picked whichever was
  // farthest out instead of the one actually facing B.
  const xCands = [];
  if (xBand.walls.length) {
    const wPlus = nearestWall(xBand.walls, fb.minx);
    xCands.push({ axis: 'x', dir: 1, wall: wPlus, delta: wPlus - fb.minx });
    const wMinus = nearestWall(xBand.walls, fb.maxx);
    xCands.push({ axis: 'x', dir: -1, wall: wMinus, delta: wMinus - fb.maxx });
  }
  const zCands = [];
  if (zBand.walls.length) {
    const wPlus = nearestWall(zBand.walls, fb.minz);
    zCands.push({ axis: 'z', dir: 1, wall: wPlus, delta: wPlus - fb.minz });
    const wMinus = nearestWall(zBand.walls, fb.maxz);
  ...
  // Only treat this as "nest into the corner on both axes" when B was
  // already dropped close to BOTH walls -- if one axis has a large gap, the
  // user clearly isn't aiming at that wall and we must not yank B across
  // the plate to force a corner fit (that would fight ordinary single-wall
  // placement, e.g. sliding a piece along an open channel).
  ...
  if (bothClose && xConcave && zConcave) {
    // A genuine 2D pocket/corner notch: A presents a real inner wall on BOTH
    // axes within B's current footprint (e.g. the notch formed where two
    // already-joined pieces meet at a corner). Close both axes onto A's
    // inner walls instead of fixing one and freezing the other -- freezing
    // the tangent axis here is what let B keep overlapping A's other wall
    // (the "blending" behavior) even though Align reported success.
  ...
  // Neither axis has any of A's geometry in B's current band -- B is fully
  // outside A on both axes (the plain diagonal/outer-corner case). Original
  // behavior, unchanged.
  const dx = ((fb.minx + fb.maxx) - (fa.minx + fa.maxx)) / 2;
  const dz = ((fb.minz + fb.maxz) - (fa.minz + fa.maxz)) / 2;
  if (Math.abs(dx) >= Math.abs(dz)) {
