...
function undoLast() {
  ...
  if (entry.type === 'softenReplace' || entry.type === 'sealReplace') {
    ...
    renderModelList();
    updateUndoBtn();
    setStatus(entry.type === 'sealReplace' ? 'Undo: Seal reverted' : 'Undo: Soften reverted');
    return;
  }
...
function NSO_seatFlushBitToHull(hullMesh, bitMesh, opts) {
  ...
  for (var it = 0; it < iters; it++) {
    var frame = NSO_plugFrame(bitMesh, hullCenter);
    ...
    var frame2 = NSO_plugFrame(bitMesh, hullCenter);
...
function seatFlushBitToHull() {
  ...
  pushUndo(snapshotPlacedPose(state.placed.indexOf(placedB)));
  const res = NSO_seatFlushBitToHull(placedA.mesh, placedB.mesh, { proud: 0.4 });
  if (!res || !res.ok) {
