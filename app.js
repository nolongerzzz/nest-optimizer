...
function buildCutHelper() {
  ...
  const hitGeo = new THREE.PlaneGeometry(pw + 4, ph + 4);
...
function updateJoinUI() {
  ...
  if (nameA) nameA.textContent = (state.joinSession && state.editId && a) ? a.name : 'Pick A, then click a piece';
  if (nameB) nameB.textContent = b ? b.name : 'Pick B, then click a piece';
  if (slotA) {
  ...
  if (step) {
    ...
    else step.textContent = '4. Complete Join';
  }
  paintJoinHighlights();
}
...
function assignJoinClick(id) {
  ...
  if (state.joinArmed === 'a') {
    if (id === state.joinPartnerId) state.joinPartnerId = null;
    state.editId = id;
    state.cutT = 0.5;
