  ...
  el.querySelectorAll('.model-item').forEach(row => {
    row.addEventListener('click', (e) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        if (id === state.editId) {
          setStatus('Pick a different piece as the Join partner', true);
          return;
        }
        state.joinPartnerId = (state.joinPartnerId === id) ? null : id;
        renderModelList();
        setStatus(state.joinPartnerId != null
          ? 'Join partner set - click Join selected'
          : 'Join partner cleared');
        return;
      }
      state.editId = id;
      state.cutT = 0.5;
