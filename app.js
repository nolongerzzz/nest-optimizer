...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  for (var sf = 0; sf < samples.length; sf++) {
    ...
    var thick = null, tOuter = null, tInner = null;
    for (var hi = 0; hi + 1 < hits.length; hi++) {
      var dt = hits[hi + 1] - hits[hi];
      if (dt >= 0.35 && dt <= 8.0) { thick = dt; tOuter = hits[hi]; tInner = hits[hi + 1]; break; }
    }
    if (thick == null) {
      if (hits.length >= 1) { tOuter = hits[0]; thick = 1.0; tInner = tOuter + thick; }
      else continue;
    }
    goodSamples++;
    if (thick < minClearance) { minClearance = thick; worstSample = sf; }
    var depth = Math.max(0.45, thick - minWall);
  ...
  if (!goodSamples || safeFarCoord == null) {
    minClearance = 1.0;
    safeFarCoord = cap + sign * 0.8;
    console.warn('[subtract] no shell pair; punch 0.8mm from cap');
  }
  var clampedDepth = false;

  var extBox = frame.box.clone();
  if (sign > 0) {
    var rawFar = extBox.max.getComponent(axis) + overshoot;
    var farA = Math.min(rawFar, safeFarCoord);
    if (farA < rawFar - tol) clampedDepth = true;
    extBox.max.setComponent(axis, farA);
  } else {
    var rawFar2 = extBox.min.getComponent(axis) - overshoot;
    var farB = Math.max(rawFar2, safeFarCoord);
    if (farB > rawFar2 + tol) clampedDepth = true;
    extBox.min.setComponent(axis, farB);
  }

  /* NEW: pad the two lateral (non-punch) axes so a seat-flush epsilon or
     a hull skin that's inset a hair from the render mesh can't produce a
     false "no overlap" on the SAT test below. Punch axis is untouched. */
  var lateralPad = Math.max(0.5, frame.span * 0.01) + tol;
  var axB = (axis + 1) % 3, axC = (axis + 2) % 3;
  extBox.min.setComponent(axB, extBox.min.getComponent(axB) - lateralPad);
  extBox.max.setComponent(axB, extBox.max.getComponent(axB) + lateralPad);
  extBox.min.setComponent(axC, extBox.min.getComponent(axC) - lateralPad);
  extBox.max.setComponent(axC, extBox.max.getComponent(axC) + lateralPad);

  var out = [];
  var toLocal = frame.inv, toWorld = frame.matrix;
  var la = new THREE.Vector3(), lb = new THREE.Vector3(), lc = new THREE.Vector3();
  var frag = [], wv = new THREE.Vector3();

  for (var i = 0; i < aWorld.length; i += 9) {
    la.set(aWorld[i], aWorld[i + 1], aWorld[i + 2]).applyMatrix4(toLocal);
    lb.set(aWorld[i + 3], aWorld[i + 4], aWorld[i + 5]).applyMatrix4(toLocal);
    lc.set(aWorld[i + 6], aWorld[i + 7], aWorld[i + 8]).applyMatrix4(toLocal);

    if (Math.max(la.x, lb.x, lc.x) < extBox.min.x || Math.min(la.x, lb.x, lc.x) > extBox.max.x ||
        Math.max(la.y, lb.y, lc.y) < extBox.min.y || Math.min(la.y, lb.y, lc.y) > extBox.max.y ||
        Math.max(la.z, lb.z, lc.z) < extBox.min.z || Math.min(la.z, lb.z, lc.z) > extBox.max.z) {
      for (var kh = 0; kh < 9; kh++) out.push(aWorld[i + kh]);
      continue;
    }
    frag.length = 0;
    NSO_clipTriOutsideAABB(la, lb, lc, extBox, frag);
    for (var f = 0; f < frag.length; f += 3) {
      wv.set(frag[f], frag[f + 1], frag[f + 2]).applyMatrix4(toWorld);
      out.push(wv.x, wv.y, wv.z);
    }
  }
  var hullKept = (out.length / 9) | 0;
  ...
  if (hullKept === 0) return { ok: false, soup: aWorld, mode: 'box', reason: 'all hull tris removed' };
  if (hullKept >= na && clampedDepth) {
    console.warn('[subtract] clamp missed skin; retry full bit depth');
    if (sign > 0) extBox.max.setComponent(axis, frame.box.max.getComponent(axis) + overshoot);
    else extBox.min.setComponent(axis, frame.box.min.getComponent(axis) - overshoot);
    out.length = 0;
    for (var i2 = 0; i2 < aWorld.length; i2 += 9) {
      la.set(aWorld[i2], aWorld[i2 + 1], aWorld[i2 + 2]).applyMatrix4(toLocal);
      lb.set(aWorld[i2 + 3], aWorld[i2 + 4], aWorld[i2 + 5]).applyMatrix4(toLocal);
      lc.set(aWorld[i2 + 6], aWorld[i2 + 7], aWorld[i2 + 8]).applyMatrix4(toLocal);
      if (Math.max(la.x, lb.x, lc.x) < extBox.min.x || Math.min(la.x, lb.x, lc.x) > extBox.max.x ||
          Math.max(la.y, lb.y, lc.y) < extBox.min.y || Math.min(la.y, lb.y, lc.y) > extBox.max.y ||
          Math.max(la.z, lb.z, lc.z) < extBox.min.z || Math.min(la.z, lb.z, lc.z) > extBox.max.z) {
        for (var kh2 = 0; kh2 < 9; kh2++) out.push(aWorld[i2 + kh2]);
        continue;
      }
      frag.length = 0;
      NSO_clipTriOutsideAABB(la, lb, lc, extBox, frag);
      for (var f2 = 0; f2 < frag.length; f2 += 3) {
        wv.set(frag[f2], frag[f2 + 1], frag[f2 + 2]).applyMatrix4(toWorld);
        out.push(wv.x, wv.y, wv.z);
      }
    }
    hullKept = (out.length / 9) | 0;
    clampedDepth = false;
  }
  if (hullKept >= na) {
    /* NEW: diagnostic — tells you exactly which axis is separating,
       instead of a bare status string. Cheap: reuses aWorld/toLocal. */
    ...
    console.warn('[subtract] extBox', extBox.min.toArray(), extBox.max.toArray(),
                 'hull-local bounds', hMin.toArray(), hMax.toArray(), 'axis', axis, 'sign', sign);
    return { ok: false, soup: aWorld, mode: 'box', reason: 'plug missed hull (no skin cut)' };
  }
