...
function dragMovePlaced(event) {
  ...
  const hit = hitPlateXZ();
  if (!hit) return;
  let nx = hit.x + state.moveGrab.x;
  let nz = hit.z + state.moveGrab.z;
  const idA = state.editId;
  const idB = state.joinPartnerId;
  const slidingB = state.joinSession && idB != null && p.sourceId === idB;
  if (slidingB && state.joinSlideAxis) {
    if (state.joinSlideAxis === 'z') nx = p.x;
    else nz = p.z;
  }
  if (slidingB && idA != null) {
    const placedA = state.placed.find(function (q) { return q && q.sourceId === idA && q.mesh; });
    if (placedA && placedA.mesh && p.mesh) {
      const bbA = new THREE.Box3().setFromObject(placedA.mesh);
      const halfX = (p.mesh.geometry && p.mesh.geometry.boundingBox)
        ? (p.mesh.geometry.boundingBox.max.x - p.mesh.geometry.boundingBox.min.x) / 2
        : (p.size ? p.size.x / 2 : 10);
      const halfZ = (p.mesh.geometry && p.mesh.geometry.boundingBox)
        ? (p.mesh.geometry.boundingBox.max.z - p.mesh.geometry.boundingBox.min.z) / 2
        : (p.size ? p.size.z / 2 : 10);
      if (!p.mesh.geometry.boundingBox && p.mesh.geometry.computeBoundingBox) {
        p.mesh.geometry.computeBoundingBox();
      }
      const bbP = p.mesh.geometry.boundingBox;
      const hx = bbP ? (bbP.max.x - bbP.min.x) / 2 : halfX;
      const hz = bbP ? (bbP.max.z - bbP.min.z) / 2 : halfZ;
      const axis = state.joinSlideAxis === 'z' ? 'x' : (state.joinSlideAxis === 'x' ? 'z' : detectMateAxis(placedA, p));
      if (axis === 'x') {
        const left = bbA.min.x - hx;
        const right = bbA.max.x + hx;
        if (p.x <= (bbA.min.x + bbA.max.x) / 2) nx = Math.min(nx, left);
        else nx = Math.max(nx, right);
      } else {
        const back = bbA.min.z - hz;
        const front = bbA.max.z + hz;
        if (p.z <= (bbA.min.z + bbA.max.z) / 2) nz = Math.min(nz, back);
        else nz = Math.max(nz, front);
      }
    }
  }
  applyPlacedXZ(p, nx, nz);
}
