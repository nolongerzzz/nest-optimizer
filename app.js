...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  /* ---------- degrade path: unchanged centroid filter ---------- */
  if (mode !== 'box') {
    var outC = [];
  ...
  /* how far past the cap the skin sits (buried face -> positive overshoot) */
  ...
  /* --- 1. hull: keep fragments outside the extended box --- */
  var out = [];
  ...
  /* --- 2. plug: drop outer cap, extend cap-plane verts, clip to skin --- */
  ...
  for (var j = 0; j < bWorld.length; j += 9) {
    ...
    /* outer cap face -> never emitted, so the mouth stays open */
    ...
    for (var w = 0; w < wallSink.length; w += 9) {
      /* reversed winding: normals face into the pocket void */
  ...
  if (hullKept >= na) return { ok: false, soup: aWorld, mode: 'box', reason: 'plug missed hull (no skin cut)' };
