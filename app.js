...
function selectPlaced(idx) {
  ...
  if (p && p.sourceId != null) {
    ...
    if (m) {
      const draggingJoinBit = !!(state.joinSession && state.joinHullId != null && p.sourceId === state.joinPartnerId);
      if (!draggingJoinBit) {
        state.editId = m.id;
        if (state.joinSession && !state.joinPartnerId) state.joinHullId = m.id;
      }
      updateEditSize();
      renderModelList();
      if (state.cutterOpen) {
...
function seatFlushBitToHull() {
  ...
  pushUndo(snapshotPlacedPose(state.placed.indexOf(placedB)));
  const res = NSO_seatFlushBitToHull(placedA.mesh, placedB.mesh, { lip: 0.2 });
  if (!res || !res.ok) {
  ...
  setStatus('Seated - 0.2mm lip, skin error ' + resid.toFixed(2) + 'mm - Subtract');
}
...
function reportJoinFlushGap() {
  ...
  const idA = state.editId, idB = state.joinPartnerId;
...
function applyXrayToMaterial(mat) {
  ...
  mat.opacity = on ? 0.92 : (mat.userData.solidOpacity != null ? mat.userData.solidOpacity : 1);
  mat.depthWrite = !on;
  if (on) {
