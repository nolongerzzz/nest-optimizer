...
function getCutPlaneForSplit(model) {
  ...
  if (Math.max(ax, az) < 1e-4) {
    return null;
  }
  const clipAxis = az >= ax ? 'z' : 'x';

  const bbox = new THREE.Box3().setFromBufferAttribute(m.geometry.attributes.position);
  const origin = clipAxis === 'x' ? bbox.min.x : bbox.min.z;
  const span = clipAxis === 'x' ? (bbox.max.x - bbox.min.x) : (bbox.max.z - bbox.min.z);
  if (!(span > 0)) return null;

  const plane = clipAxis === 'x' ? worldPoint.x : worldPoint.z;
