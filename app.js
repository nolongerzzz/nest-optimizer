...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  for (var sf = 0; sf < samples.length; sf++) {
    ...
    var hits = NSO_wallHitsAlong(aWorld, ox, oy, oz, punchDir, Math.max(frame.span * 3, 60));
    if (hits.length < 2) {
      return { ok: false, soup: aWorld, mode: 'box',
        reason: 'no far-wall hit at sample ' + sf + ' - refusing, cannot guarantee blind pocket' };
    }
    var thick = hits[1] - hits[0];
...
function subtractBFromA() {
  ...
  try {
    ...
    if (res.clamped) {
      window.__nestSubtractMsg = 'Subtract ok - pocket depth clamped to ' +
        res.minClearance.toFixed(2) + 'mm - min ' + res.minWall.toFixed(2) + 'mm to preserve far wall.';
    } else {
      window.__nestSubtractMsg = 'Subtract ok';
    }
  } catch (err) {
...
function extractBitFromSelected() {
  ...
  const parts = [];
  let filled = 0;
  for (let x = x0 + half; x <= x1 - half + 1e-6; x += step) {
    for (let y = y0 + half; y <= y1 - half + 1e-6; y += step) {
      for (let z = z0 + half; z <= z1 - half + 1e-6; z += step) {
        if (NSO_pointInsideSoup(soupPiece, x, y, z)) continue;
        filled++;
        const mn = { x: x - half, y: y - half, z: z - half };
        const mx = { x: x + half, y: y + half, z: z + half };
        const cube = soupAxisBox(mn, mx);
        for (let i = 0; i < cube.length; i++) parts.push(cube[i]);
      }
    }
  }
  if (filled < 4) {
  ...
  let bit = weldSoupVerts(new Float32Array(parts), 0.15);
  if (typeof capSmallOpenLoops === 'function') bit = capSmallOpenLoops(bit, 3);
  if (!bit || bit.length < 9) {
    setStatus('Extract failed - no cavity found', true);
    return;
  }
  const geo = soupToCenteredGeo(bit);
  geo.computeBoundingBox();
  const size = new THREE.Vector3();
  ...
  setStatus('Extract bit ok - ' + filled + ' cells, ' + size.x.toFixed(0) + 'x' + size.y.toFixed(0) + 'x' + size.z.toFixed(0) + ' mm');
}
