...
function jigBoxesFor(q) {
  ...
  const t = 0.7;
  return [
    { minx: fp.minx - t, maxx: fp.minx, minz: fp.minz, maxz: fp.maxz },
    { minx: fp.maxx, maxx: fp.maxx + t, minz: fp.minz, maxz: fp.maxz },
    { minx: fp.minx, maxx: fp.maxx, minz: fp.minz - t, maxz: fp.minz },
    { minx: fp.minx, maxx: fp.maxx, minz: fp.maxz, maxz: fp.maxz + t }
  ];
}
...
function resolveDragPose(p, selfIdx, fromX, fromZ, toX, toZ) {
  const wall = clampToApproachWalls(p, selfIdx, fromX, fromZ, toX, toZ);
  toX = wall.x;
  toZ = wall.z;
...
function joinHalvesOnPlate(modelA, placedA, modelB, placedB, axisIdx) {
  ...
  const isLJoin = overlapT < Math.min(spanL, spanR) * 0.8;
  let leftKept = left;
  let rightKept = right;
  if (!isLJoin) {
