...
function snapPlacedOffNeighbors(p, selfIdx, onlySourceId) {
  if (!p || !p.mesh || !state.placed) return;
  const SNAP = 3.2;
  const REACH = 8;
  ...
  for (let i = 0; i < state.placed.length; i++) {
    ...
    if (onlySourceId != null && q.sourceId !== onlySourceId) continue;
    const other = new THREE.Box3().setFromObject(q.mesh);
    const overlapX = Math.min(bb.max.x, other.max.x) - Math.max(bb.min.x, other.min.x);
    const overlapZ = Math.min(bb.max.z, other.max.z) - Math.max(bb.min.z, other.min.z);
    if (overlapX > 0 && overlapZ > 0) {
      if (overlapX <= overlapZ) {
        dx += (bb.min.x + bb.max.x < other.min.x + other.max.x) ? -overlapX : overlapX;
      } else {
        dz += (bb.min.z + bb.max.z < other.min.z + other.max.z) ? -overlapZ : overlapZ;
      }
    }
    if (overlapX <= 0 && overlapX > -SNAP && overlapZ > -REACH) {
      dx += (bb.max.x <= other.min.x) ? -overlapX : overlapX;
    }
    if (overlapZ <= 0 && overlapZ > -SNAP && overlapX > -REACH) {
      dz += (bb.max.z <= other.min.z) ? -overlapZ : overlapZ;
    }
  }
  if (Math.abs(dx) > 0.01 || Math.abs(dz) > 0.01) applyPlacedXZ(p, p.x + dx, p.z + dz);
}
...
function dragMovePlaced(event) {
  ...
  let only = null;
  if (state.joinSession && p.sourceId != null) {
    if (p.sourceId === state.editId) only = state.joinPartnerId;
    else if (p.sourceId === state.joinPartnerId) only = state.editId;
  }
  snapPlacedOffNeighbors(p, state.moveIndex, only);
}
