...
function alignJoinForSlide() {
  ...
  const candidates = [];
  if (xBand.hits) {
    candidates.push({ axis: 'x', delta: xBand.max - fb.minx });
    candidates.push({ axis: 'x', delta: xBand.min - fb.maxx });
  }
  if (zBand.hits) {
    candidates.push({ axis: 'z', delta: zBand.max - fb.minz });
    candidates.push({ axis: 'z', delta: zBand.min - fb.maxz });
  }
  ...
  if (candidates.length) {
    ...
    matchPlacedBottoms(placedA, placedB);
    setStatus("Aligned to A's inner wall");
    return true;
  }
