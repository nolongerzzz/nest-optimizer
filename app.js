...
function updateCutHelper() {
  ...
  const mesh = placed ? placed.mesh : state.previewMesh;
  const span = info.axis === 'x' ? m.size.x : m.size.z;
  const along = -span / 2 + info.t * span;
  if (mesh) {
    state.cutHelper.position.copy(mesh.position);
    state.cutHelper.quaternion.copy(mesh.quaternion);
    if (info.axis === 'x') state.cutHelper.translateX(along);
    else state.cutHelper.translateZ(along);
  } else {
    const wp = { x: 0, y: 8, z: 0 };
    if (info.axis === 'x') state.cutHelper.position.set(along, wp.y, 0);
    else state.cutHelper.position.set(0, wp.y, along);
...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  /* extended cut box: plug AABB pushed outward along the punch axis */
  var extBox = frame.box.clone();
  if (sign > 0) extBox.max.setComponent(axis, extBox.max.getComponent(axis) + overshoot);
  else extBox.min.setComponent(axis, extBox.min.getComponent(axis) - overshoot);
