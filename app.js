...
function meshBandExtent(mesh, axis, tMin, tMax, pad) {
  // Extent of A's ACTUAL solid along `axis` ('x' or 'z'), restricted to the
  // band [tMin,tMax] on the OTHER (tangent) axis -- found by clipping every
  // triangle to the band, not by filtering vertices (a flat face's vertices
  // sit only at its corners, so a band between corners would see nothing).
  // This is what makes a concave L's real inner wall visible: A's bbox has
  // no idea the notch exists, but A's triangles restricted to the exact
  // band B occupies do.
  const out = { min: Infinity, max: -Infinity, hits: false };
  ...
  for (let ti = 0; ti < triCount; ti++) {
    ...
    for (let k = 0; k < poly.length; k++) {
      if (poly[k].n < out.min) out.min = poly[k].n;
      if (poly[k].n > out.max) out.max = poly[k].n;
    }
  }
  return out;
}
...
function alignJoinForSlide() {
  ...
  const fa = placedFootprint(placedA);
  const fb = placedFootprint(placedB);
  const CONCAVE_TOL = 1.0; // mm -- real print features (chamfers, ribs) can

  const xBand = meshBandExtent(placedA.mesh, 'x', fb.minz, fb.maxz);
  const zBand = meshBandExtent(placedA.mesh, 'z', fb.minx, fb.maxx);
  const candidates = [];
  if (xBand.hits) {
    candidates.push({ axis: 'x', dir: 1, wall: xBand.max, delta: xBand.max - fb.minx });
    candidates.push({ axis: 'x', dir: -1, wall: xBand.min, delta: xBand.min - fb.maxx });
  }
  if (zBand.hits) {
    candidates.push({ axis: 'z', dir: 1, wall: zBand.max, delta: zBand.max - fb.minz });
    candidates.push({ axis: 'z', dir: -1, wall: zBand.min, delta: zBand.min - fb.maxz });
  }
  ...
  if (candidates.length) {
    candidates.sort(function (c1, c2) { return Math.abs(c1.delta) - Math.abs(c2.delta); });
    const best = candidates[0];
    const globalWall = best.axis === 'x'
      ? (best.dir > 0 ? fa.maxx : fa.minx)
      : (best.dir > 0 ? fa.maxz : fa.minz);
    const concaveHere = Math.abs(best.wall - globalWall) > CONCAVE_TOL;
