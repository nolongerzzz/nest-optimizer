...
function undoLast() {
  ...
  if (entry.type === 'softenReplace' || entry.type === 'sealReplace' || entry.type === 'solidifyReplace' || entry.type === 'thickenReplace') {
    ...
    renderModelList();
    updateUndoBtn();
    setStatus(entry.type === 'thickenReplace' ? 'Undo: Thicken reverted' : (entry.type === 'solidifyReplace' ? 'Undo: Solidify reverted' : (entry.type === 'sealReplace' ? 'Undo: Seal reverted' : 'Undo: Soften reverted')));
    return;
  }
...
function applySoftenOnFace(face) {
  ...
  let working = null;
  try {
    working = softenSelectedFace(m.rawTris, face.axisIdx, keepMinFace, R);
  } catch (e) {
    working = null;
  }
  if (!working || !rawCheckWatertightQuick(working)) {
    setStatus('Soften failed - that face did not fillet clean. Piece unchanged', true);
    return;
  }
...
function softenSelectedModel() {
  ...
  setStatus('Soften: click the end or face to round');
}
...
function updateJoinUI() {
  ...
  if (btnSub) btnSub.disabled = !(state.joinSession && state.editId && state.joinPartnerId && state.editId !== state.joinPartnerId);
...
/* ============================================================
   ...
   ============================================================ */
function subtractSoupBFromA(aWorld, bWorld, opts) {
  opts = opts || {};
  ...
  var mode = opts.mode || (frame ? 'box' : 'centroid');
  if (mode === 'box' && !frame) mode = 'centroid';

  if (mode !== 'box') {
    var outC = [];
    for (var i2 = 0; i2 < aWorld.length; i2 += 9) {
      var cx2 = (aWorld[i2] + aWorld[i2 + 3] + aWorld[i2 + 6]) / 3;
      var cy2 = (aWorld[i2 + 1] + aWorld[i2 + 4] + aWorld[i2 + 7]) / 3;
      var cz2 = (aWorld[i2 + 2] + aWorld[i2 + 5] + aWorld[i2 + 8]) / 3;
      if (!NSO_pointInsideSoup(bWorld, cx2, cy2, cz2)) {
        for (var kk = 0; kk < 9; kk++) outC.push(aWorld[i2 + kk]);
      }
    }
    var keptC = (outC.length / 9) | 0, wallC = 0;
    for (var j2 = 0; j2 < bWorld.length; j2 += 9) {
      var bx2 = (bWorld[j2] + bWorld[j2 + 3] + bWorld[j2 + 6]) / 3;
      var by2 = (bWorld[j2 + 1] + bWorld[j2 + 4] + bWorld[j2 + 7]) / 3;
      var bz2 = (bWorld[j2 + 2] + bWorld[j2 + 5] + bWorld[j2 + 8]) / 3;
      if (!NSO_pointInsideSoup(aWorld, bx2, by2, bz2)) continue;
      outC.push(bWorld[j2], bWorld[j2 + 1], bWorld[j2 + 2],
                bWorld[j2 + 6], bWorld[j2 + 7], bWorld[j2 + 8],
                bWorld[j2 + 3], bWorld[j2 + 4], bWorld[j2 + 5]);
      wallC++;
    }
    if (!keptC || !wallC) return { ok: false, soup: aWorld, mode: 'centroid', reason: 'centroid path produced no pocket' };
    return { ok: true, soup: new Float32Array(outC), mode: 'centroid', hullTris: keptC, wallTris: wallC };
  }

  var grid = NSO_buildHullGrid(aWorld);
  var axis = frame.axis, sign = frame.sign, cap = frame.capCoord;
  var tol = Math.max(1e-4, frame.span * 1e-3);
  var toLocal = frame.inv, toWorld = frame.matrix;
  var axB = (axis + 1) % 3, axC = (axis + 2) % 3;

  var samples = NSO_outerFaceCorners(frame);
  var mid = new THREE.Vector3();
  for (var s0 = 0; s0 < 4; s0++) mid.add(samples[s0]);
  mid.multiplyScalar(0.25);
  samples.push(mid);

  // Densify: 4 edge-midpoints on the footprint, so a local thin feature
  // that the 5 corner/center points straddle isn't missed entirely.
  var bMin = frame.box.min.getComponent(axB), bMax = frame.box.max.getComponent(axB);
  var cMin = frame.box.min.getComponent(axC), cMax = frame.box.max.getComponent(axC);
  var bMid = (bMin + bMax) * 0.5, cMid = (cMin + cMax) * 0.5;
  var edgeLocalPts = [[bMid, cMin], [bMid, cMax], [bMin, cMid], [bMax, cMid]];
  for (var ep = 0; ep < edgeLocalPts.length; ep++) {
    var lv = new THREE.Vector3();
    lv.setComponent(axis, cap);
    lv.setComponent(axB, edgeLocalPts[ep][0]);
    lv.setComponent(axC, edgeLocalPts[ep][1]);
    samples.push(lv.applyMatrix4(frame.matrix));
  }

  var overshoot = 0;
  for (var s = 0; s < samples.length; s++) {
    var rr0 = NSO_signedDistToHull(grid, samples[s].x, samples[s].y, samples[s].z);
    if (rr0 && rr0.d < 0) overshoot = Math.max(overshoot, -rr0.d);
  }
  overshoot += Math.max(0.5, frame.span * 0.02);

  var minWall = (opts.minWall === undefined) ? 1.0 : opts.minWall;
  var safetyMargin = 0.15;
  var minClearance = Infinity, worstSample = -1;
  var punchDir = frame.punch || new THREE.Vector3(
    axis === 0 ? sign : 0, axis === 1 ? sign : 0, axis === 2 ? sign : 0
  );

  var goodSamples = 0;
  var safeFarCoord = null;
  var toLocalEarly = frame.inv;
  var rayBackoff = Math.max(1.0, overshoot); // was a fixed 2.0 -- now scaled to this hull's own geometry

  for (var sf = 0; sf < samples.length; sf++) {
    var ox = samples[sf].x - punchDir.x * rayBackoff;
    var oy = samples[sf].y - punchDir.y * rayBackoff;
    var oz = samples[sf].z - punchDir.z * rayBackoff;
    var hits = NSO_wallHitsAlong(aWorld, ox, oy, oz, punchDir, Math.max(frame.span * 3, 80));
    if (hits.length < 1) continue;

    var tOuter = hits[0];
    var thick = null;
    if (hits.length >= 2) {
      var dt = hits[1] - tOuter;
      if (dt >= 0.35 && dt <= 8.0) thick = dt;
    }
    if (thick == null) continue;

    var maxDepth = thick - safetyMargin;
    if (maxDepth <= 0) continue;

    goodSamples++;
    if (thick < minClearance) { minClearance = thick; worstSample = sf; }

    var depth = Math.min(Math.max(0.2, thick - minWall), maxDepth);
    var farWx = ox + punchDir.x * (tOuter + depth);
    var farWy = oy + punchDir.y * (tOuter + depth);
    var farWz = oz + punchDir.z * (tOuter + depth);
    var farL = new THREE.Vector3(farWx, farWy, farWz).applyMatrix4(toLocalEarly);
    var fc = farL.getComponent(axis);
    if (safeFarCoord == null) safeFarCoord = fc;
    else if (sign > 0) safeFarCoord = Math.min(safeFarCoord, fc);
    else safeFarCoord = Math.max(safeFarCoord, fc);
  }

  var usedFallbackGuess = false;
  if (!goodSamples || safeFarCoord == null) {
    usedFallbackGuess = true;
    // Bit-scaled default instead of an arbitrary 0.8mm -- still just a
    // starting point, the staged-deepen/verify loop below is what actually
    // trusts it.
    var fallbackDepth = Math.min(Math.max(frame.span * 0.15, 1.5), 5.0);
    safeFarCoord = cap + sign * fallbackDepth;
    minClearance = null;
    console.warn('[subtract] no reliable shell pair from any of ' + samples.length +
      ' samples; starting staged probe at ' + fallbackDepth.toFixed(2) + 'mm (bit-scaled) from cap');
  }

  function buildExtBox(farCoord) {
    var eb = frame.box.clone();
    var clampedByBound = false;
    if (sign > 0) {
      var rawFar = frame.box.max.getComponent(axis) + overshoot;
      var farA = Math.min(rawFar, farCoord);
      if (farA < rawFar - tol) clampedByBound = true;
      eb.max.setComponent(axis, farA);
    } else {
      var rawFar2 = frame.box.min.getComponent(axis) - overshoot;
      var farB = Math.max(rawFar2, farCoord);
      if (farB > rawFar2 + tol) clampedByBound = true;
      eb.min.setComponent(axis, farB);
    }
    var lateralPad = Math.max(0.5, frame.span * 0.01) + tol;
    eb.min.setComponent(axB, eb.min.getComponent(axB) - lateralPad);
    eb.max.setComponent(axB, eb.max.getComponent(axB) + lateralPad);
    eb.min.setComponent(axC, eb.min.getComponent(axC) - lateralPad);
    eb.max.setComponent(axC, eb.max.getComponent(axC) + lateralPad);
    return { box: eb, clampedByBound: clampedByBound };
  }

  function clipHullAgainstBox(eb) {
    var localOut = [];
    var la2 = new THREE.Vector3(), lb2 = new THREE.Vector3(), lc2 = new THREE.Vector3();
    var frag2 = [], wv2 = new THREE.Vector3();
    for (var ii = 0; ii < aWorld.length; ii += 9) {
      la2.set(aWorld[ii], aWorld[ii + 1], aWorld[ii + 2]).applyMatrix4(toLocal);
      lb2.set(aWorld[ii + 3], aWorld[ii + 4], aWorld[ii + 5]).applyMatrix4(toLocal);
      lc2.set(aWorld[ii + 6], aWorld[ii + 7], aWorld[ii + 8]).applyMatrix4(toLocal);
      if (Math.max(la2.x, lb2.x, lc2.x) < eb.min.x || Math.min(la2.x, lb2.x, lc2.x) > eb.max.x ||
          Math.max(la2.y, lb2.y, lc2.y) < eb.min.y || Math.min(la2.y, lb2.y, lc2.y) > eb.max.y ||
          Math.max(la2.z, lb2.z, lc2.z) < eb.min.z || Math.min(la2.z, lb2.z, lc2.z) > eb.max.z) {
        for (var kh = 0; kh < 9; kh++) localOut.push(aWorld[ii + kh]);
        continue;
      }
      frag2.length = 0;
      NSO_clipTriOutsideAABB(la2, lb2, lc2, eb, frag2);
      for (var f = 0; f < frag2.length; f += 3) {
        wv2.set(frag2[f], frag2[f + 1], frag2[f + 2]).applyMatrix4(toWorld);
        localOut.push(wv2.x, wv2.y, wv2.z);
      }
    }
    return localOut;
  }

  var maxSteps = 3, stepSize = 0.5;
  var out = null, hullKept = 0, extBox = null, clampedDepth = false, attemptsUsed = 0;

  for (var attempt = 0; attempt <= maxSteps; attempt++) {
    var tryCoord = safeFarCoord + sign * stepSize * attempt;
    var built = buildExtBox(tryCoord);
    var candidateOut = clipHullAgainstBox(built.box);
    var candidateKept = (candidateOut.length / 9) | 0;
    if (candidateKept > 0 && candidateKept < na) {
      out = candidateOut;
      hullKept = candidateKept;
      extBox = built.box;
      clampedDepth = built.clampedByBound || usedFallbackGuess || attempt > 0;
  ...
  if (out == null) {
    var hMin = new THREE.Vector3(Infinity, Infinity, Infinity);
    var hMax = new THREE.Vector3(-Infinity, -Infinity, -Infinity);
    var dv = new THREE.Vector3();
    for (var di = 0; di < aWorld.length; di += 3) {
      dv.set(aWorld[di], aWorld[di + 1], aWorld[di + 2]).applyMatrix4(toLocal);
      hMin.min(dv); hMax.max(dv);
    }
    console.warn('[subtract] staged deepen exhausted (' + maxSteps + ' steps), no full-bit fallback -- hull-local bounds',
      hMin.toArray(), hMax.toArray(), 'axis', axis, 'sign', sign);
    if (typeof setStatus === 'function') setStatus('Blind pocket failed: no safe wall found after staged probe -- A left unchanged');
    return { ok: false, soup: aWorld, mode: 'box', reason: 'plug missed hull after staged deepen (no full-bit fallback)' };
  }

  // Re-anchor the cap push to where the pocket ACTUALLY bottomed out, not
  // to the unrelated `overshoot` value. This is what was leaving the back
  // of the pocket unsealed.
  var depthUsed = (sign > 0) ? (extBox.max.getComponent(axis) - cap) : (cap - extBox.min.getComponent(axis));
  var capPush = depthUsed + Math.max(0.05, tol * 2);

  var pa = new THREE.Vector3(), pb = new THREE.Vector3(), pc = new THREE.Vector3();
  var wallSink = [];
  var capDropped = 0, wall = 0;

  function onCap(v) { return Math.abs(v.getComponent(axis) - cap) < tol; }
  function extend(v) {
    if (onCap(v)) v.setComponent(axis, v.getComponent(axis) + sign * capPush);
    return v;
  }

  for (var j = 0; j < bWorld.length; j += 9) {
    pa.set(bWorld[j], bWorld[j + 1], bWorld[j + 2]).applyMatrix4(toLocal);
    pb.set(bWorld[j + 3], bWorld[j + 4], bWorld[j + 5]).applyMatrix4(toLocal);
    pc.set(bWorld[j + 6], bWorld[j + 7], bWorld[j + 8]).applyMatrix4(toLocal);

    if (onCap(pa) && onCap(pb) && onCap(pc)) { capDropped++; continue; }

    extend(pa); extend(pb); extend(pc);
    pa.applyMatrix4(toWorld); pb.applyMatrix4(toWorld); pc.applyMatrix4(toWorld);

    wallSink.length = 0;
    NSO_clipTriInsideHull(grid, pa, pb, pc, wallSink);
    for (var w = 0; w < wallSink.length; w += 9) {
      out.push(wallSink[w], wallSink[w + 1], wallSink[w + 2],
               wallSink[w + 6], wallSink[w + 7], wallSink[w + 8],
               wallSink[w + 3], wallSink[w + 4], wallSink[w + 5]);
      wall++;
    }
  }

  if (wall === 0) return { ok: false, soup: aWorld, mode: 'box', reason: 'no plug wall inside hull' };

  var wallReport = (minClearance != null) ? minClearance.toFixed(2) + 'mm' : 'unmeasured';
  if (typeof setStatus === 'function') {
    setStatus('Blind pocket cut: wall ' + wallReport +
      (clampedDepth ? ' (clamped)' : '') +
      (attemptsUsed > 0 ? ', deepened ' + attemptsUsed + '\u00d70.5mm' : ''));
  }

  return {
    ok: true,
    soup: new Float32Array(out),
    mode: 'box',
    hullTris: hullKept,
    wallTris: wall,
    capDropped: capDropped,
    overshoot: overshoot,
    depthUsed: depthUsed,
    clamped: clampedDepth,
    minClearance: minClearance,
    minWall: minWall,
    deepenSteps: attemptsUsed
  };
}

function subtractBFromA() {
  const idA = state.editId;
  const idB = state.joinPartnerId;
  if (!state.joinSession || idA == null || idB == null || idA === idB) {
  ...
  try {
    ...
    const res = subtractSoupBFromA(aWorld, bWorld, {
      plugFrame: frame,
      mode: 'box'
    });
    ...
    console.log('[subtract]', res.mode, 'hull', res.hullTris, 'walls', res.wallTris);
    window.__nestSubtractMsg = 'Subtract ok - wall ' +
      (res.minClearance != null ? Number(res.minClearance).toFixed(2) : '?') +
      'mm' + (res.clamped ? ' (clamped)' : '') +
      (res.deepenSteps ? (', deepen ' + res.deepenSteps) : '');
  } catch (err) {
...
function seatFlushBitToHull() {
  ...
  pushUndo(snapshotPlacedPose(state.placed.indexOf(placedB)));
  const res = NSO_seatFlushBitToHull(placedA.mesh, placedB.mesh, { proud: 2.2 });
  if (!res || !res.ok) {
  ...
  setStatus('Seated flush - slide, then Subtract');
}
...
function setupUI() {
  ...
  if (btnSubtract) btnSubtract.addEventListener('click', subtractBFromA);
...
function applyXrayToMaterial(mat) {
  ...
  mat.opacity = on ? 0.22 : (mat.userData.solidOpacity != null ? mat.userData.solidOpacity : 1);
  mat.depthWrite = !on;
