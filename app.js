...
function closeCutter(silent) {
  ...
  removeCutHelper();
  state.previewMesh = null;
  // Restore every library model on the plate (halves included)
  showAllModelsOnPlate();
  updateCutterUI();
  if (!silent) {
    setStatus(
      state.models.length > 1
        ? ('Cutter closed — ' + state.models.length + ' pieces on plate. Optimize to pack multi-up.')
        : 'Cutter closed — Optimize when ready'
    );
  }
}
