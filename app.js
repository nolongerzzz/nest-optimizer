...
function renderModelList() {
  ...
  el.querySelectorAll('.model-item').forEach(row => {
    row.addEventListener('click', (e) => {
      ...
      if (state.joinPickMode) {
        if (id === state.editId) {
          setStatus('Mark B: click a DIFFERENT piece than A', true);
          return;
        }
        state.joinPartnerId = id;
        state.joinPickMode = false;
        renderModelList();
        setStatus('B = ' + (state.models.find(function(x){return x.id===id;})||{}).name + ' - click Join selected');
        return;
      }
      state.editId = id;
      if (state.joinPartnerId === id) state.joinPartnerId = null;
      state.cutT = 0.5;
      renderModelList();
      updateEditSize();
      if (state.cutterOpen) {
    ...
    if (modelHits.length) {
      ...
      if (typeof idx === 'number' && state.placed[idx]) {
        ...
        if (p.sourceId != null && p.sourceId !== state.editId) {
          setOrbitFromPlate(false);
          if (state.joinPickMode) {
            state.joinPartnerId = p.sourceId;
            state.joinPickMode = false;
            renderModelList();
            setStatus('B set - click Join selected');
            event.stopPropagation();
            return;
          }
          selectPlaced(idx);
...
function setupUI() {
  ...
  const btnMarkB = document.getElementById('btn-join-mark-b');
  if (btnMarkB) btnMarkB.addEventListener('click', startMarkJoinPartner);
