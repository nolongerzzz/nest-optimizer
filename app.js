...
function capFromEdges(edges, axis, plane, keepMin) {
  // Seal hollow cut faces: weld boundary edges → closed loops → 2D ear-clip fill.
  if (!edges.length) return [];

  const Q = 50; // 0.02 mm quantize
  function keyOf(p) {
  ...
  function to2(p) {
    // plane coords: u across, v = height (Y)
  ...
  const pts = new Map(); // key → [x,y,z]
  const adj = new Map(); // key → [neighbor keys]
  ...
  // Walk every unused edge into loops
  const used = new Set();
  ...
  for (const startKey of adj.keys()) {
    for (const nb of (adj.get(startKey) || [])) {
      if (used.has(ekey(startKey, nb))) continue;
      const loop = [startKey];
      let prev = startKey;
      let cur = nb;
      ...
      while (guard++ < 10000) {
        loop.push(cur);
        if (cur === startKey && loop.length > 2) { closed = true; break; }
        const nbs = adj.get(cur) || [];
        let next = null;
        // Prefer degree-2 chain; otherwise first unused
        for (let i = 0; i < nbs.length; i++) {
          const cand = nbs[i];
          if (cand === prev) continue;
          if (!used.has(ekey(cur, cand))) { next = cand; break; }
        }
        if (!next) break;
      ...
      if (loop[loop.length - 1] === startKey) loop.pop();
      if (loop.length >= 3) loops.push(loop);
    }
  }

  // Ear-clip a simple 2D polygon (indices into coords array)
  function earclip(coords2) {
    const n0 = coords2.length;
    if (n0 < 3) return [];
    const idx = [];
    ...
    function area2(i, j, k) {
      const a = coords2[i], b = coords2[j], c = coords2[k];
      return (b[0] - a[0]) * (c[1] - a[1]) - (b[1] - a[1]) * (c[0] - a[0]);
    }
    function inside(i, j, k, p) {
      const a = coords2[i], b = coords2[j], c = coords2[k], d = coords2[p];
      const b0 = (b[0] - a[0]) * (d[1] - a[1]) - (b[1] - a[1]) * (d[0] - a[0]);
      const b1 = (c[0] - b[0]) * (d[1] - b[1]) - (c[1] - b[1]) * (d[0] - b[0]);
      const b2 = (a[0] - c[0]) * (d[1] - c[1]) - (a[1] - c[1]) * (d[0] - c[0]);
      const hasNeg = b0 < -1e-12 || b1 < -1e-12 || b2 < -1e-12;
      const hasPos = b0 > 1e-12 || b1 > 1e-12 || b2 > 1e-12;
      return !(hasNeg && hasPos);
    }
    // Ensure CCW
    let signed = 0;
    for (let i = 0; i < n0; i++) {
      const a = coords2[i], b = coords2[(i + 1) % n0];
    ...
    while (idx.length > 3 && guard++ < 5000) {
      ...
      for (let i = 0; i < idx.length; i++) {
        ...
        if (area2(i0, i1, i2) <= 1e-12) continue; // not a convex ear
        let anyIn = false;
        for (let j = 0; j < idx.length; j++) {
          const p = idx[j];
          if (p === i0 || p === i1 || p === i2) continue;
          if (inside(i0, i1, i2, p)) { anyIn = true; break; }
        }
        if (anyIn) continue;
        tris.push([i0, i1, i2]);
        idx.splice(i, 1);
  ...
  for (const loop of loops) {
    const coords3 = loop.map(k => pts.get(k)).filter(Boolean);
    if (coords3.length < 3) continue;
    const coords2 = coords3.map(to2);
    const tris = earclip(coords2);
    if (!tris.length) {
      // Fallback fan from centroid
      let cx = 0, cy = 0;
      coords2.forEach(p => { cx += p[0]; cy += p[1]; });
      cx /= coords2.length; cy /= coords2.length;
      const c3 = from2(cx, cy);
      for (let i = 0; i < coords3.length; i++) {
        const b = coords3[i];
        const d = coords3[(i + 1) % coords3.length];
        const abx = b[0] - c3[0], aby = b[1] - c3[1], abz = b[2] - c3[2];
        const acx = d[0] - c3[0], acy = d[1] - c3[1], acz = d[2] - c3[2];
        const nx = aby * acz - abz * acy;
        const ny = abz * acx - abx * acz;
        const nz = abx * acy - aby * acx;
        const flip = nx * desired.x + ny * desired.y + nz * desired.z < 0;
        if (flip) out.push(c3[0], c3[1], c3[2], d[0], d[1], d[2], b[0], b[1], b[2]);
        else out.push(c3[0], c3[1], c3[2], b[0], b[1], b[2], d[0], d[1], d[2]);
      }
      continue;
    }
    for (const t of tris) {
      const a = coords3[t[0]], b = coords3[t[1]], c = coords3[t[2]];
      const abx = b[0] - a[0], aby = b[1] - a[1], abz = b[2] - a[2];
      const acx = c[0] - a[0], acy = c[1] - a[1], acz = c[2] - a[2];
      const nx = aby * acz - abz * acy;
      const ny = abz * acx - abx * acz;
