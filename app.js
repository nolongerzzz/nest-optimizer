...
/** Single source of truth: red plane position in geometry local space */
function getCutPlaneLocal(model) {
  ...
  const bbox = new THREE.Box3().setFromBufferAttribute(m.geometry.attributes.position);
  const span = axis === 'x' ? (bbox.max.x - bbox.min.x) : (bbox.max.z - bbox.min.z);
  // Prefer live bbox span over cached size so plane matches the mesh
  const t = state.cutT;
  if (axis === 'x') {
    return { axis, plane: bbox.min.x + t * span, bbox, span };
  }
  return { axis, plane: bbox.min.z + t * span, bbox, span };
}
