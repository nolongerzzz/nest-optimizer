...
function undoLast() {
  ...
  if (entry.type === 'softenReplace' || entry.type === 'sealReplace' || entry.type === 'solidifyReplace') {
    ...
    renderModelList();
    updateUndoBtn();
    setStatus(entry.type === 'solidifyReplace' ? 'Undo: Solidify reverted' : (entry.type === 'sealReplace' ? 'Undo: Seal reverted' : 'Undo: Soften reverted'));
    return;
  }
...
function solidifySelectedModel() {
  ...
  try { openAfter = openBoundaryEdges(working).length; } catch (e) {}
  if (openBefore != null && openAfter != null) {
    const closed = openBefore - openAfter;
    const suffix = openAfter > 0
      ? ' (' + closed + ' closed, ' + openAfter + ' kept open as slots)'
      : ' (' + closed + ' closed)';
    setStatus('Solidify ok - open edges ' + openBefore + '\u2192' + openAfter + suffix);
  } else {
