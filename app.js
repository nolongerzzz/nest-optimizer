...
function updateAdjustUI() {
  ...
  ['btn-rot-left', 'btn-rot-right', 'btn-flip', 'btn-tip', 'btn-roll', 'btn-raise', 'btn-lower', 'btn-nudge-left', 'btn-nudge-right', 'btn-nudge-fwd', 'btn-nudge-back', 'btn-delete-placed', 'btn-frame-selected']
    .forEach(id => {
...
function subtractSoupBFromA(soupA, soupB) {
  ...
  if (!kept.length) return null; // bit engulfed the whole hull -- fail safe

  let result = weldSoupVerts(new Float32Array(kept), 0.3);
  if (result.length < 9) return null;

  // Close only weld-tolerance slivers left by the clip. capSmallOpenLoops
  // is span-gated (default 3mm) and explicitly leaves bigger loops alone,
  // so the bit's actual cavity/slot opening is never capped closed.
  if (typeof capSmallOpenLoops === 'function') {
