...
function seatFlushBitToHull() {
  ...
  pushUndo(snapshotPlacedPose(state.placed.indexOf(placedB)));
  const res = NSO_seatFlushBitToHull(placedA.mesh, placedB.mesh, { proud: 0.5 });
  if (!res || !res.ok) {
  ...
  setStatus('Seated - proud ' + proudMm.toFixed(1) + 'mm, skin error ' + resid.toFixed(2) + 'mm - Subtract');
}
