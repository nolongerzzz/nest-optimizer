...
function showEditPreview() {
  ...
  if (state.controls && state.camera) {
    const maxDim = Math.max(m.size.x, m.size.y, m.size.z, 40);
    state.controls.target.set(0, m.size.y / 2, 0);
    state.camera.position.set(maxDim * 1.6, maxDim * 1.4, maxDim * 1.6);
    state.controls.update();
  }
  syncCutUI();
...
function clipGeometrySide(geometry, axis, plane, keepMin) {
  ...
  function keep(v) {
    return keepMin ? v <= plane + 1e-6 : v >= plane - 1e-6;
  }
...
function cutActiveModel() {
  ...
  if (cutMm < 1 || cutMm > span - 1) {
    setStatus('Slide the red plane off the very ends (need ≥1 mm each side)', true);
    return;
  }
  const bbox = new THREE.Box3().setFromBufferAttribute(m.geometry.attributes.position);
  ...
  addModel(m.name + '-A' + tag, left);
  addModel(m.name + '-B' + tag, right);
  setStatus('Split into two square ends at ' + cutMm.toFixed(1) + ' mm on ' + axis.toUpperCase() + '. Original kept.');
}
