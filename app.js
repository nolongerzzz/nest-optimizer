...
function initThree() {
  ...
  state.renderer.setClearColor(0x0a0c10, 1);
...
function renderModelList() {
  ...
  state.models.forEach(m => {
    ...
    const tag = (m.id === state.editId) ? '<span class="tag tag-a">A</span>'
      : (m.id === state.joinPartnerId) ? '<span class="tag tag-b">B</span>' : '';
    row.innerHTML = `
      ${tag}
  ...
  el.querySelectorAll('.model-item').forEach(row => {
    row.addEventListener('click', (e) => {
      ...
      // Ctrl/Cmd+click marks this piece as the Join partner instead of
      // changing the active selection - lets you pick A normally, then
      // mark B without losing A.
      assignJoinClick(id);
      return;
      state.cutT = 0.5;
      renderModelList();
  ...
  if (state.cutterOpen && state.modelGroup) {
    ...
    if (modelHits.length) {
      ...
      if (typeof idx === 'number' && state.placed[idx]) {
        const p = state.placed[idx];
        if (p.sourceId != null && p.sourceId !== state.editId) {
          setOrbitFromPlate(false);
          assignJoinClick(p.sourceId);
          event.stopPropagation();
          return;
          event.stopPropagation();
          return;
  ...
  if (state.modelGroup) {
    ...
    if (modelHits.length) {
      ...
      if (typeof idx === 'number' && state.placed.length) {
        ...
        if (hit && hit.sourceId != null) assignJoinClick(hit.sourceId);
        selectPlaced(idx);
...
function layoutUndoModels(models) {
  ...
  state.placed.forEach(function (pl, i) {
    if (pl.mesh && pl.mesh.material && pl.mesh.material.color) {
      pl.mesh.material.color.setHex(colors[i % colors.length]);
    }
  });
  const exportBtn = document.getElementById('btn-export-stl');
  if (exportBtn) exportBtn.disabled = state.placed.length === 0;
...
function layoutAfterSplit(sourcePose, poseById, sourceId, modelA, modelB, axis) {
  if (!state.modelGroup) return;
  clearDisplayMeshes();
  state.placed = [];
  state.previewMesh = null;
  ...
  state.models.forEach(function (mod) {
    ...
    const pose = poseById[mod.id] || { x: 0, z: 0 };
    placeModelMovable(mod, pose.x, pose.z);
  });

  const wA = (modelA && modelA.size) ? modelA.size.x : 20;
  const wB = (modelB && modelB.size) ? modelB.size.x : 20;
  const xLeft = sourcePose.x - wB / 2 - gap / 2;
  const xRight = sourcePose.x + wA / 2 + gap / 2;
  if (modelB) placeModelMovable(modelB, xLeft, sourcePose.z);
  if (modelA) placeModelMovable(modelA, xRight, sourcePose.z);

  if (state.models.length === 2) {
    state.editId = state.models[0].id;
    state.joinPartnerId = state.models[1].id;
  }
...
function rawResultToDisplayGeometry(rawFlatTris) {
...
function rawJoinPieces(rawMin, rawMax, axisIdx) {
  ...
  const joinPlane = bbMin.maxV; // close the gap onto the min piece's own face
  const delta = joinPlane - bbMax.minV;
  const maxMoved = rawJoinTranslateAxis(rawMax, axisIdx, delta);

  const { tris: minStripped, stripped: sMin } = rawJoinStripCapAtPlane(rawMin, axisIdx, joinPlane);
  const { tris: maxStripped, stripped: sMax } = rawJoinStripCapAtPlane(maxMoved, axisIdx, joinPlane);
  if (sMin === 0 || sMax === 0) throw new Error('no matching flat interface found between the two pieces');
  ...
  if (!rawCheckWatertightQuick(merged)) throw new Error('joined result is not watertight');
  return merged;
}
...
function paintJoinHighlights() {
  (state.placed || []).forEach(function (pl) {
    ...
    if (pl.sourceId === state.editId) {
      pl.mesh.material.color.setHex(0xfb923c);
      pl.mesh.material.emissive = new THREE.Color(0x9a3412);
      pl.mesh.material.emissiveIntensity = 0.45;
    } else if (pl.sourceId === state.joinPartnerId) {
      pl.mesh.material.color.setHex(0x4ade80);
      pl.mesh.material.emissive = new THREE.Color(0x14532d);
      pl.mesh.material.emissiveIntensity = 0.45;
    } else {
      pl.mesh.material.emissive = new THREE.Color(0x000000);
...
function updateJoinUI() {
  const a = state.models.find(function (x) { return x.id === state.editId; });
  ...
  if (nameA) nameA.textContent = a ? a.name : 'Click first piece';
  if (nameB) nameB.textContent = b ? b.name : 'Click second piece';
  if (slotA) slotA.classList.toggle('filled-a', !!a);
  if (slotB) slotB.classList.toggle('filled-b', !!b);
  if (btn) btn.disabled = !(a && b && a.id !== b.id);
  if (step) {
    if (!a) step.textContent = '1. Click the first piece — it becomes A (orange).';
    else if (!b) step.textContent = '2. Click the other piece — it becomes B (green).';
    else step.textContent = '3. Click Join A + B into one piece.';
  }
  paintJoinHighlights();
}

function assignJoinClick(id) {
  if (id == null) return;
  if (state.editId == null) {
    state.editId = id;
    state.cutT = 0.5;
  } else if (state.joinPartnerId == null && id !== state.editId) {
    state.joinPartnerId = id;
  } else if (id === state.editId) {
    state.editId = state.joinPartnerId;
    state.joinPartnerId = null;
    state.cutT = 0.5;
  } else if (id === state.joinPartnerId) {
    state.joinPartnerId = null;
  } else {
    state.joinPartnerId = id;
  }
  ...
  if (state.cutterOpen) {
    const m = getActiveModel();
    const placed = state.placed.find(function (p) { return p && p.sourceId === state.editId; });
    if (placed && placed.mesh) {
      removeCutHelper();
      state.previewMesh = placed.mesh;
      buildCutHelper();
    } else if (m) {
      showEditPreview();
    }
...
function clearJoinSlots() {
  state.editId = state.models.length ? state.models[0].id : null;
  state.joinPartnerId = null;
  state.joinPickMode = false;
  renderModelList();
  updateEditSize();
  updateJoinUI();
  setStatus('Join slots cleared');
}

function startMarkJoinPartner() {
  const a = getActiveModel();
  if (!a) {
    setStatus('Click piece A in the list first, then Mark B', true);
    return;
  }
  state.joinPickMode = true;
  setStatus('Mark B: now click the OTHER piece');
}

function joinSelectedModels() {
  let idA = state.editId;
  let idB = state.joinPartnerId;
  if (idA == null && state.models.length === 2) idA = state.models[0].id;
  if (idB == null && state.models.length === 2) {
    idB = state.models[0].id === idA ? state.models[1].id : state.models[0].id;
  }
  if (idA == null || idB == null || idA === idB) {
    setStatus('Click piece A, click Mark B, click the other piece, then Join', true);
    return;
  }
  ...
  const rawMin = aIsMin ? rawA : rawB;
  const rawMax = aIsMin ? rawB : rawA;

  let joined;
  try {
    joined = rawJoinPieces(rawMin, rawMax, 0);
  } catch (err) {
    console.warn('[join] failed:', err.message);
    setStatus('Join failed - pieces unchanged', true);
    return;
  }
  ...
  const newGeo = rawResultToDisplayGeometry(joined);
  newGeo.computeBoundingBox();
  const size2 = new THREE.Vector3();
  newGeo.boundingBox.getSize(size2);

  modelA.geometry = newGeo;
  modelA.rawTris = joined;
  modelA.rawAxis = 'zup';
  modelA.centerOffset = computeCenterOffsetFromRaw(joined);
  ...
  // Keep A at its plate X/Z — rebuild its mesh in place.
  if (placedA) {
    const px = placedA.x, pz = placedA.z;
    if (placedA.mesh && state.modelGroup) {
      state.modelGroup.remove(placedA.mesh);
...
function splitBothSides(model, axis, leftPlane, rightPlane) {
  try {
    ...
    return {
      left: rawResultToDisplayGeometry(rawL),
      right: rawResultToDisplayGeometry(rawR),
      engine: 'raw',
      // Each half's own raw mesh data, so a SECOND Split on either half can
      // also use rawCut instead of falling back to clipGeometrySide.
      rawA: rawL,
    ...
    console.warn('[rawCut] both halves fallback to clipGeometrySide:', err.message);
    return {
...
function cutActiveModel() {
  ...
  removeCutHelper();
  state.previewMesh = null;
  layoutAfterSplit(sourcePose, poseById, sourceId, modelA, modelB, axis);
  const target = getCutterTargetMesh();
  if (target) {
    state.previewMesh = target;
    state.cutT = 0.5;
