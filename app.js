...
function NSO_plugFrame(bitMesh, hullCenterWorld) {
  ...
  var outward = new THREE.Vector3().subVectors(plugCenterWorld, hullCenterWorld);
  if (outward.lengthSq() < 1e-12) outward.set(0, 1, 0);
  outward.normalize();

  var nm = new THREE.Matrix3().getNormalMatrix(M);
  var bestDot = -Infinity, axis = 1, sign = 1, bestN = new THREE.Vector3(0, 1, 0);
  var cand = new THREE.Vector3();
  for (var a = 0; a < 3; a++) {
    for (var s = -1; s <= 1; s += 2) {
      cand.set(0, 0, 0).setComponent(a, s).applyMatrix3(nm).normalize();
      var d = cand.dot(outward);
      if (d > bestDot) { bestDot = d; axis = a; sign = s; bestN.copy(cand); }
    }
  }
  ...
  return {
    ...
    axis: axis,
    sign: sign,
    outerNormal: bestN.clone(),          // world, points away from hull
    punch: bestN.clone().negate(),        // world, inward
...
/* four corners of the outer face, in cycle order, world space */
...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  for (var sf = 0; sf < samples.length; sf++) {
    ...
    for (var hi = 0; hi + 1 < hits.length; hi++) {
      ...
      if (dt >= 0.35 && dt <= 8.0) { thick = dt; break; }
    }
  ...
  if (!goodSamples) {
    minClearance = 2.0;
    worstSample = -1;
    console.warn('[subtract] no shell pair; assuming 2mm wall');
  }

  if (minClearance - minWall <= 0) {
    return { ok: false, soup: aWorld, mode: 'box',
      reason: 'wall too thin for bit (' + minClearance.toFixed(2) + 'mm < minWall ' +
              minWall.toFixed(2) + 'mm) at sample ' + worstSample };
  }

  var safeFarCoord = cap + sign * (minClearance - minWall);
  ...
  if (hullKept >= na && clampedDepth) {
    console.warn('[subtract] clamp missed skin; retry unclamped');
...
function subtractBFromA() {
  ...
  try {
    ...
    const frame = NSO_plugFrame(placedB.mesh, hullCenter);
