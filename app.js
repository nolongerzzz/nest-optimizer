...
function cloneSelectedModel() {
  ...
  const gap = Math.max(src.size.x, 10) + 3;
  let baseX = 0;
  if (state.placed.length) {
    const maxX = Math.max(...state.placed.map(p => p.x + p.width / 2));
    baseX = maxX + gap / 2;
  }
  const newIds = [];
  for (let i = 0; i < n; i++) {
    ...
    newIds.push(id);
    const model = state.models.find(x => x.id === id);
    if (model) placeModelMovable(model, baseX + i * gap, 0);
  }
  if (newIds.length) {
  ...
  updateEditSize();
  updateUndoBtn();
  setStatus('Cloned ' + newIds.length + ' × ' + src.name + ' onto plate');
}
...
function capFromEdges(edges, axis, plane, keepMin) {
  ...
  const rChamfer = Math.max(0, Math.min(1.2, CHAMFER_MM));
  ...
  /** Inward offset of a 2D loop (CCW). Miter with limit; acute corners → bevel. */
  function offsetLoop2D(loop3, dist) {
    if (loop3.length < 3 || dist < 1e-6) return null;
    const n = loop3.length;
    const p2 = loop3.map(p => {
      const t = to2(p);
      return { u: t[0], v: t[1] };
    });
    // Ensure CCW
    ...
    for (let i = 0; i < n; i++) {
      const j = (i + 1) % n;
      area += p2[i].u * p2[j].v - p2[j].u * p2[i].v;
    }
    const pts = area < 0 ? p2.slice().reverse() : p2.slice();
    const nn = pts.length;
    const out2 = [];

    for (let i = 0; i < nn; i++) {
      const prev = pts[(i + nn - 1) % nn];
      const curr = pts[i];
      const next = pts[(i + 1) % nn];
      // Edge directions
      let e1u = curr.u - prev.u, e1v = curr.v - prev.v;
      let e2u = next.u - curr.u, e2v = next.v - curr.v;
      const l1 = Math.hypot(e1u, e1v) || 1;
      const l2 = Math.hypot(e2u, e2v) || 1;
      e1u /= l1; e1v /= l1;
      e2u /= l2; e2v /= l2;
      // Inward normals (left of CCW edge)
      const n1u = -e1v, n1v = e1u;
      const n2u = -e2v, n2v = e2u;
      // Miter direction
      let mu = n1u + n2u, mv = n1v + n2v;
      const ml = Math.hypot(mu, mv);
      if (ml < 1e-8) {
        out2.push({ u: curr.u + n1u * dist, v: curr.v + n1v * dist });
        continue;
      }
      mu /= ml; mv /= ml;
      // cos(half) ≈ dot(miter, n1)
      const cosH = Math.max(0.15, Math.abs(mu * n1u + mv * n1v));
      let miterLen = dist / cosH;
      if (miterLen > dist * CHAMFER_MITER) {
        // Bevel fallback: two points along edge normals
        out2.push({ u: curr.u + n1u * dist, v: curr.v + n1v * dist });
        out2.push({ u: curr.u + n2u * dist, v: curr.v + n2v * dist });
      } else {
        out2.push({ u: curr.u + mu * miterLen, v: curr.v + mv * miterLen });
      }
    }
    // Reject if self-intersecting or collapsed (simple segment-cross check)
    if (out2.length < 3) return null;
    function segCross(a, b, c, d) {
      function cross(ax, ay, bx, by) { return ax * by - ay * bx; }
      const d1 = cross(b.u - a.u, b.v - a.v, c.u - a.u, c.v - a.v);
      const d2 = cross(b.u - a.u, b.v - a.v, d.u - a.u, d.v - a.v);
      const d3 = cross(d.u - c.u, d.v - c.v, a.u - c.u, a.v - c.v);
      const d4 = cross(d.u - c.u, d.v - c.v, b.u - c.u, b.v - c.v);
      return d1 * d2 < 0 && d3 * d4 < 0;
    }
    const m = out2.length;
    let crosses = 0;
    for (let i = 0; i < m; i++) {
      const a = out2[i], b = out2[(i + 1) % m];
      for (let j = i + 2; j < m; j++) {
        if (i === 0 && j === m - 1) continue;
        const c = out2[j], d = out2[(j + 1) % m];
        if ((j + 1) % m === i) continue;
        if (segCross(a, b, c, d)) crosses++;
      }
    }
    // Allow a couple of local glitches; only abort if clearly tangled
    if (crosses > 3) return null;
    return out2.map(q => from2(q.u, q.v));
  }

  function shiftAlongAxis(p, delta) {
    const q = [p[0], p[1], p[2]];
    if (axis === 'x') q[0] += delta;
    else if (axis === 'y') q[1] += delta;
    else q[2] += delta;
    return q;
  }

  loops.forEach(loop => {
    let usedChamfer = false;
    const outer = loop.map(p => snap(p));
    if (rChamfer > 0.05 && outer.length >= 3) {
      // Try decreasing radii until offset succeeds
      const tryRadii = [rChamfer, rChamfer * 0.75, rChamfer * 0.5, 0.4, 0.3];
      for (let ti = 0; ti < tryRadii.length && !usedChamfer; ti++) {
        const r = tryRadii[ti];
        if (r < 0.2) break;
        const innerFlat = offsetLoop2D(outer, r);
        if (!innerFlat || innerFlat.length < 3) continue;
        const into = -outward * r;
        const inner = innerFlat.map(p => shiftAlongAxis(snap(p), into));
        // Same vertex count path: rebuild offset to match outer count via edge midnormals
        if (inner.length >= 3) {
          const tris = earClip(inner);
          if (!tris.length) continue;
          tris.forEach(t => pushOriented(t[0], t[1], t[2]));
          // Bridge: sample both loops at equal parameter
          const no = outer.length;
          const ni = inner.length;
          for (let i = 0; i < no; i++) {
            const o0 = outer[i];
            const o1 = outer[(i + 1) % no];
            const t0 = i / no;
            const t1 = ((i + 1) % no) / no;
            const i0 = inner[Math.min(ni - 1, Math.round(t0 * ni) % ni)];
            const i1 = inner[Math.min(ni - 1, Math.round(t1 * ni) % ni)];
            pushOriented(o0, o1, i1);
            pushOriented(o0, i1, i0);
          }
          usedChamfer = true;
        }
      }
    ...
    if (!usedChamfer) {
      // Guaranteed soft-ish fallback: push whole cap slightly into material
      // and add a thin perimeter rim (still better than razor when it prints)
      const into = -outward * Math.max(0.35, rChamfer * 0.5);
      const pushed = outer.map(p => shiftAlongAxis(p, into));
      const tris = earClip(pushed);
      tris.forEach(t => pushOriented(t[0], t[1], t[2]));
      for (let i = 0; i < outer.length; i++) {
        const o0 = outer[i];
        const o1 = outer[(i + 1) % outer.length];
        const i0 = pushed[i];
        const i1 = pushed[(i + 1) % pushed.length];
        pushOriented(o0, o1, i1);
        pushOriented(o0, i1, i0);
      }
    }
