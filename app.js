...
// ===================== Orientation =====================
// density = max models, but reject knife-edge/tip poses
...
function scoreOrientation(geometry, mode = 'density') {
  ...
  let score;
  if (mode === 'density') {
    // Pack tightly, but punish unprintable tip/edge orientations
    score =
      footprint * 0.55 +
      maxSide * 3.5 +
      height * 0.6 +
      (baseAspect > 3.5 ? (baseAspect - 3.5) * 100 : 0) +
      (stability < 0.2 ? 250 : 0) +
      (height > maxSide * 1.6 ? height * 2.5 : 0);
  } else {
    score =
      height * 3.0 -
      footprint * 0.01 +
      maxSide * 0.2 +
      (baseAspect > 3 ? (baseAspect - 3) * 50 : 0);
  }
  return { score, size, footprint, height, maxSide, baseAspect, stability };
}
  ...
  try {
    ...
    function topOrients(geometry, mode, n = 4) {
      ...
      rotations.forEach(([rx, ry, rz]) => {
        ...
        scored.push({ geometry: geo, size: result.size, score: result.score });
      });
