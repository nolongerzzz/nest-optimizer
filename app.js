...
function extractBitFromSelected() {
  ...
  const bb = meshLocalBox3(p.mesh);
  const pad = 0.15;
  const box = soupAxisBox(
    { x: bb.min.x - pad, y: bb.min.y - pad, z: bb.min.z - pad },
    { x: bb.max.x + pad, y: bb.max.y + pad, z: bb.max.z + pad }
  );
  let bit = null;
  try {
    bit = subtractSoupBFromA(box, soupPiece);
  } catch (err) {
    setStatus('Extract failed - piece unchanged', true);
    return;
  }
  ...
  const bitVol = size.x * size.y * size.z;
  if (bitVol > boxVol * 0.85) {
    setStatus('Extract failed - tile is not a block with a hole', true);
    return;
  }
  ...
  const x = p.x + (p.width || size.x) / 2 + size.x / 2 + 6;
  const m = state.models.find(function (x) { return x.id === id; });
  if (m) placeModelMovable(m, x, p.z);
  setStatus('Extract bit ok - ' + size.x.toFixed(0) + 'x' + size.y.toFixed(0) + 'x' + size.z.toFixed(0) + ' mm');
}
