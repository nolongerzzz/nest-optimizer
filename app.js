  ...
  for (let pass = 0; pass < 4; pass++) {
    ...
    for (let i = 0; i < state.placed.length; i++) {
      ...
      if (overlapX > 0 && overlapZ > 0) {
        if (overlapX <= overlapZ) dx += (bb.min.x + bb.max.x <= other.min.x + other.max.x) ? -overlapX : overlapX;
        else dz += (bb.min.z + bb.max.z <= other.min.z + other.max.z) ? -overlapZ : overlapZ;
        continue;
      }
      if (overlapX <= 0 && overlapX > -SNAP && overlapZ > 0) {
        dx += (bb.max.x <= other.min.x) ? -overlapX : overlapX;
      }
      if (overlapZ <= 0 && overlapZ > -SNAP && overlapX > 0) {
        dz += (bb.max.z <= other.min.z) ? -overlapZ : overlapZ;
      }
    }
    if (Math.abs(dx) < 0.01 && Math.abs(dz) < 0.01) break;
