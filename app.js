...
function cloneSelectedModel() {
  ...
  const cellW = w + gap;
  const cellD = d + gap;
  // How many fit across the plate in X
  ...
  for (let i = 0; i < n; i++) {
    ...
    const gridW = totalCols * cellW - gap;
    const gridD = totalRows * cellD - gap;
    const x0 = -gridW / 2 + w / 2;
    const z0 = -gridD / 2 + d / 2;
    const x = x0 + col * cellW;
    const z = z0 + row * cellD;
    placeModelMovable(model, x, z);
  }
  if (newIds.length) {
