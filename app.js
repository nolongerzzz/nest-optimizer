...
function rawFilletCut(rawTris, axisIdx, plane, keepMin, requestedR) {
  ...
  const marginLoop2d = simplifyCollinear2D(outerMargin3d.map(p => [p[other[0]], p[other[1]]]));
  ...
  const ring0 = matchToRing0(marginLoop2d, computeRing(0));
  ...
  const snapMargin = function (uv) {
    const c = marginCanonical.get(Math.round(uv[0] / stitchTol) + '|' + Math.round(uv[1] / stitchTol));
    return c ? [c[other[0]], c[other[1]]] : uv;
  };
  for (const [p1, p2, p3] of stitch) {
    const A = from3(snapMargin(p1), marginPlane);
    const B = from3(snapMargin(p2), marginPlane);
    const C = from3(snapMargin(p3), marginPlane);
    const ux=B[0]-A[0], uy=B[1]-A[1], uz=B[2]-A[2];
    const vx=C[0]-A[0], vy=C[1]-A[1], vz=C[2]-A[2];
