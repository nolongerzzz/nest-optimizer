...
function captureJoinFace(hit) {
  ...
  if (Math.abs(n.y) >= Math.abs(n.x) && Math.abs(n.y) >= Math.abs(n.z)) {
    setStatus('Click the cut wall, not the top');
    return null;
  }
...
function assignJoinClick(id, face) {
  ...
  const useFace = state.joinUseFaces ? (face || null) : null;
...
function setupUI() {
  ...
  if (btnJoinFaces) btnJoinFaces.addEventListener('click', function () {
    state.joinUseFaces = !state.joinUseFaces;
    if (!state.joinUseFaces) {
      state.joinFaceA = null;
      state.joinFaceB = null;
      removeFaceHelper();
    }
    updateJoinUI();
    setStatus(state.joinUseFaces
      ? 'Faces ON — click the cut wall on A, then on B'
      : 'Auto join — pick two pieces');
  });
  const slotA = document.getElementById('join-slot-a');
