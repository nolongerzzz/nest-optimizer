...
function updateAdjustUI() {
  ...
  ['btn-rot-left', 'btn-rot-right', 'btn-flip', 'btn-tip', 'btn-roll', 'btn-raise', 'btn-lower', 'btn-tilt-up', 'btn-tilt-dn', 'btn-nudge-left', 'btn-nudge-right', 'btn-nudge-fwd', 'btn-nudge-back', 'btn-delete-placed', 'btn-frame-selected']
    .forEach(id => {
 ...
 */
function subtractSoupBFromA(soupA, soupB) {
  if (!soupA || soupA.length < 9) return null;
  if (!soupB || soupB.length < 9) return new Float32Array(soupA);

  let minX = Infinity, minY = Infinity, minZ = Infinity;
  let maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
  for (let i = 0; i < soupB.length; i += 3) {
    const x = soupB[i], y = soupB[i + 1], z = soupB[i + 2];
    if (x < minX) minX = x; if (x > maxX) maxX = x;
    if (y < minY) minY = y; if (y > maxY) maxY = y;
    if (z < minZ) minZ = z; if (z > maxZ) maxZ = z;
  }
  const pad = 0.5;
  const bb = { minX: minX - pad, minY: minY - pad, minZ: minZ - pad, maxX: maxX + pad, maxY: maxY + pad, maxZ: maxZ + pad };
  ...
  function insideB(px, py, pz) {
    if (!inBox(px, py, pz)) return false;
    let hits = 0;
    const n = soupB.length / 9;
    for (let t = 0; t < n; t++) {
      const i0 = t * 9;
      if (rayHitsTriangle(px, py, pz,
        soupB[i0], soupB[i0 + 1], soupB[i0 + 2],
        soupB[i0 + 3], soupB[i0 + 4], soupB[i0 + 5],
        soupB[i0 + 6], soupB[i0 + 7], soupB[i0 + 8])) hits++;
    }
    return (hits % 2) === 1;
  }

  const kept = [];
  const triCount = soupA.length / 9;
  for (let t = 0; t < triCount; t++) {
    const i0 = t * 9;
    const cx = (soupA[i0] + soupA[i0 + 3] + soupA[i0 + 6]) / 3;
    const cy = (soupA[i0 + 1] + soupA[i0 + 4] + soupA[i0 + 7]) / 3;
    const cz = (soupA[i0 + 2] + soupA[i0 + 5] + soupA[i0 + 8]) / 3;
    if (insideB(cx, cy, cz)) continue; // drop: this bit of A got cut away
    for (let k = 0; k < 9; k++) kept.push(soupA[i0 + k]);
  }
  if (!kept.length) return null;

  // Stamp B's skin as the pocket wall (flip winding). Only faces whose
  // centroid is inside A — skip the proud cap sitting outside the hull.
  const nB = soupB.length / 9;
  for (let t = 0; t < nB; t++) {
    const i0 = t * 9;
    const cx = (soupB[i0] + soupB[i0 + 3] + soupB[i0 + 6]) / 3;
    const cy = (soupB[i0 + 1] + soupB[i0 + 4] + soupB[i0 + 7]) / 3;
    const cz = (soupB[i0 + 2] + soupB[i0 + 5] + soupB[i0 + 8]) / 3;
    if (typeof pointInsideSoup === 'function') {
      if (!pointInsideSoup(soupA, cx, cy, cz)) continue;
    }
    kept.push(
      soupB[i0], soupB[i0 + 1], soupB[i0 + 2],
      soupB[i0 + 6], soupB[i0 + 7], soupB[i0 + 8],
      soupB[i0 + 3], soupB[i0 + 4], soupB[i0 + 5]
    );
  }

  let result = weldSoupVerts(new Float32Array(kept), 0.3);
  if (result.length < 9) return null;
  if (typeof capSmallOpenLoops === 'function') {
    result = capSmallOpenLoops(result, 3);
  }
  return result;
}
...
function subtractBFromA() {
  ...
  try {
    ...
    const soupA = geomToWorldSoup(modelA.geometry, placedA.x, pyA, placedA.z);
    const soupB = geomToWorldSoup(modelB.geometry, placedB.x, pyB, placedB.z);
    const cut = subtractSoupBFromA(soupA, soupB);
    if (!cut || cut.length < 9) throw new Error('result empty');
    newGeo = soupToCenteredGeo(cut);
  } catch (err) {
  ...
  // Snapshot BOTH pieces before mutating anything -- same two-piece
  // restore Join uses.
  pushUndo({
  ...
  // Remove B from the library and the plate -- matches Join deleting its partner.
  ...
  // Result replaces A at A's own pose -- no repositioning.
  if (placedA.mesh && state.modelGroup) {
    state.modelGroup.remove(placedA.mesh);
    if (placedA.mesh.material) {
      if (Array.isArray(placedA.mesh.material)) placedA.mesh.material.forEach(mt => mt.dispose());
...
function pointInsideSoup(soup, px, py, pz) {
  const DX = 0.5257311, DY = 0.6881910, DZ = 0.4998877;
  let hits = 0;
  const n = soup.length / 9;
  for (let t = 0; t < n; t++) {
    ...
    const tt = inv * (e2x * qx + e2y * qy + e2z * qz);
    if (tt > 1e-6) hits++;
  }
  return (hits % 2) === 1;
}
...
function seatFlushBitToHull() {
  ...
  const modelA = state.models.find(function (x) { return x.id === idA; });
  const modelB = state.models.find(function (x) { return x.id === idB; });
  const placedA = state.placed.find(function (p) { return p && p.sourceId === idA && p.mesh; });
  const placedB = state.placed.find(function (p) { return p && p.sourceId === idB && p.mesh; });
  if (!modelA || !modelB || !placedA || !placedB) {
    setStatus('Both hull and bit must be on the plate', true);
    return;
  }
  const soupA = geomToWorldSoup(modelA.geometry, placedA.x, placedA.mesh.position.y, placedA.z);
  if (!soupA || soupA.length < 9) {
    setStatus('Seat failed - hull empty', true);
    return;
  }
  ...
  const cz = (bbB.min.z + bbB.max.z) / 2;
  let best = null, bestD = Infinity, nx = 0, ny = 0, nz = 1;
  const ntri = soupA.length / 9;
  for (let t = 0; t < ntri; t++) {
    const i0 = t * 9;
    const ax = soupA[i0], ay = soupA[i0 + 1], az = soupA[i0 + 2];
    const bx = soupA[i0 + 3], by = soupA[i0 + 4], bz = soupA[i0 + 5];
    const cx2 = soupA[i0 + 6], cy2 = soupA[i0 + 7], cz2 = soupA[i0 + 8];
    const hit = closestPointOnTri(cx, cy, cz, ax, ay, az, bx, by, bz, cx2, cy2, cz2);
  ...
  if (!best) {
    setStatus('Seat failed - no hull surface', true);
    return;
  }
  let nlen = Math.sqrt(nx * nx + ny * ny + nz * nz);
  if (nlen < 1e-8) {
    setStatus('Seat failed - flat normal', true);
    return;
  }
  nx /= nlen; ny /= nlen; nz /= nlen;
  placedA.mesh.updateMatrixWorld(true);
  const bbA = meshLocalBox3(placedA.mesh);
  const hx = (bbA.min.x + bbA.max.x) / 2;
  const hy = (bbA.min.y + bbA.max.y) / 2;
  const hz = (bbA.min.z + bbA.max.z) / 2;
  // Outward = away from the hull center, never "toward the bit".
  if (nx * (best.x - hx) + ny * (best.y - hy) + nz * (best.z - hz) < 0) {
    nx = -nx; ny = -ny; nz = -nz;
  }
  const corners = [
    [bbB.min.x, bbB.min.y, bbB.min.z], [bbB.max.x, bbB.min.y, bbB.min.z],
    [bbB.min.x, bbB.max.y, bbB.min.z], [bbB.max.x, bbB.max.y, bbB.min.z],
    [bbB.min.x, bbB.min.y, bbB.max.z], [bbB.max.x, bbB.min.y, bbB.max.z],
    [bbB.min.x, bbB.max.y, bbB.max.z], [bbB.max.x, bbB.max.y, bbB.max.z]
  ];
  let maxDot = -Infinity, minDot = Infinity;
  for (let i = 0; i < 8; i++) {
    const d = corners[i][0] * nx + corners[i][1] * ny + corners[i][2] * nz;
    if (d > maxDot) maxDot = d;
    if (d < minDot) minDot = d;
  }
  const surfDot = best.x * nx + best.y * ny + best.z * nz;
  const span = Math.max(0.5, maxDot - minDot);
  // Outer face to the skin, then bury ~70% of the bit so it punches through
  // instead of sitting on the silhouette.
  const proud = 0.3;
  const delta = (surfDot - maxDot) - span * 0.7 + proud;
  pushUndo(snapshotPlacedPose(state.placed.indexOf(placedB)));
  placedB.x += nx * delta;
  placedB.z += nz * delta;
  placedB.liftY = (placedB.liftY || 0) + ny * delta;
  if (placedB.liftY < 0) placedB.liftY = 0;
  applyPlacedXZ(placedB, placedB.x, placedB.z);
  if (typeof applyMeshRotation === 'function') applyMeshRotation(placedB);
  setStatus('Seated into hull - slide, then Subtract');
}
