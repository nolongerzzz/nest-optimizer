...
function seatFlushBitToHull() {
  ...
  // Outward = pointing toward the bit (from hull surface to B).
  const toBx = cx - best.x, toBy = cy - best.y, toBz = cz - best.z;
  if (nx * toBx + ny * toBy + nz * toBz < 0) {
    nx = -nx; ny = -ny; nz = -nz;
  }
  // Outermost corner of B along that outward normal should sit on the surface.
  const corners = [
  ...
  setStatus('Seated flush - slide on the plate, then Subtract');
}
