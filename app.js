...
function placedOverlapsOthers(p, selfIdx) {
  ...
  for (let i = 0; i < state.placed.length; i++) {
    ...
    const ox = Math.min(bb.max.x, other.max.x) - Math.max(bb.min.x, other.min.x);
    const oz = Math.min(bb.max.z, other.max.z) - Math.max(bb.min.z, other.min.z);
    if (ox > 0.08 && oz > 0.08) return true;
  }
  return false;
}
...
function resolveDragPose(p, selfIdx, fromX, fromZ, toX, toZ) {
  ...
  if (!placedOverlapsOthers(p, selfIdx)) return;
  applyPlacedXZ(p, toX, fromZ);
  const okX = !placedOverlapsOthers(p, selfIdx);
  applyPlacedXZ(p, fromX, toZ);
  const okZ = !placedOverlapsOthers(p, selfIdx);
