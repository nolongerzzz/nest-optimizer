...
var NSO_CSG = (function () {
  ...
  function soupToManifold(wasm, soup) {
    ...
    try {
      ...
      throw new Error('Not manifold (kernel constructor)');
    }
  }
...
async function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  try {
    ...
    try {
      ...
      throw new Error('hull rejected: ' + ((eA && eA.message) ? eA.message : eA));
    }
    ...
    try {
      ...
      throw new Error('bit rejected: ' + ((eB && eB.message) ? eB.message : eB));
    }
    ...
    var noopEps = Math.max(1e-3, volBefore * 1e-4);
