...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  if (hullKept >= na && clampedDepth) {
    console.warn('[subtract] clamp missed skin; deepen 1.5mm');
    if (sign > 0) extBox.max.setComponent(axis, extBox.max.getComponent(axis) + 1.5);
    else extBox.min.setComponent(axis, extBox.min.getComponent(axis) - 1.5);
    out.length = 0;
    for (var i2 = 0; i2 < aWorld.length; i2 += 9) {
