...
const CHAMFER_MM = 0.6; // mild soft edge on cut face (bevel band)
...
function capFromEdges(edges, axis, plane, keepMin) {
  ...
  function offsetLoop2D(loop3, dist) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      for (let j = i + 2; j < m; j++) {
        ...
        if ((j + 1) % m === i) continue;
        if (segCross(a, b, c, d)) return null;
      }
    }
  ...
  loops.forEach(loop => {
    // Try soft chamfer: outer at plane, inner inset + pushed into material
    let usedChamfer = false;
    if (rChamfer > 0.05 && loop.length >= 3) {
      const inner2 = offsetLoop2D(loop, rChamfer);
      if (inner2 && inner2.length >= 3) {
        // Into material: opposite outward
        const into = -outward * rChamfer;
        const outer = loop.map(p => snap(p));
        const inner = inner2.map(p => shiftAlongAxis(snap(p), into));
        // Cap = earclip inner
        const tris = earClip(inner);
        tris.forEach(t => pushOriented(t[0], t[1], t[2]));
        // Chamfer band: bridge outer[i]–outer[i+1]–inner[i+1]–inner[i]
        const no = outer.length;
        const ni = inner.length;
        // Resample if counts differ — pair by index mod
        const n = Math.min(no, ni);
        if (n >= 3) {
          for (let i = 0; i < n; i++) {
            const o0 = outer[i];
            const o1 = outer[(i + 1) % no];
            const i0 = inner[Math.floor(i * ni / no) % ni];
            const i1 = inner[Math.floor(((i + 1) % no) * ni / no) % ni];
            // Two tris for quad — winding so outward-ish
