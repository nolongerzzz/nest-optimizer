...
function subtractSoupBFromA(aWorld, bWorld, opts) {
  ...
  var safetyMargin = 0.15; // far plane never gets closer than this to the measured inner skin
  ...
  for (var sf = 0; sf < samples.length; sf++) {
    var ox = samples[sf].x - punchDir.x * 2.0;
    var oy = samples[sf].y - punchDir.y * 2.0;
    var oz = samples[sf].z - punchDir.z * 2.0;
    var hits = NSO_wallHitsAlong(aWorld, ox, oy, oz, punchDir, Math.max(frame.span * 3, 80));
    if (hits.length < 1) continue;

    // Strict: hits[0] is always the outer skin. Only hits[1] is ever read
    // as the inner skin -- no scanning further pairs, which previously
    // could latch onto the wrong wall and make thickness look bigger
    // than it was.
    var tOuter = hits[0];
    var thick = null;
    if (hits.length >= 2) {
    ...
    if (thick == null) continue; // no reliable inner skin here -- skip, never guess

    var maxDepth = thick - safetyMargin; // hard ceiling: can't reach the inner skin
  ...
  if (!goodSamples || safeFarCoord == null) {
    ...
    safeFarCoord = cap + sign * 0.8; // unverified starting point -- must pass the check below
    minClearance = null;
    console.warn('[subtract] no reliable shell pair from any sample; starting staged probe at 0.8mm from cap');
  }
  ...
  // Staged verified deepen: 0.5mm steps, max 3 tries. Never the full bit depth.
  ...
  function extend(v) {
    if (onCap(v)) v.setComponent(axis, v.getComponent(axis) + sign * overshoot);
    return v;
  }
