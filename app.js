...
function onCanvasPointerDown(event) {
  if (!state.renderer || !state.camera || !state.modelGroup) return;
  ...
  if (state.measureOn) {
    ...
    state.measurePts.push(pt);
    const out = document.getElementById('measure-out');
    if (state.measurePts.length === 1) {
      if (out) out.textContent = 'click 2nd point';
    } else {
      const a = state.measurePts[0];
      const b = state.measurePts[1];
      const d = a.distanceTo(b);
      if (out) out.textContent = d.toFixed(2) + ' mm';
      setStatus('Measure: ' + d.toFixed(2) + ' mm');
      const pitch = document.getElementById('array-pitch');
      if (pitch && d > 1) pitch.value = d.toFixed(2);
      state.measureOn = false;
      state.measurePts = [];
      const btn = document.getElementById('btn-measure');
      if (btn) btn.textContent = 'Measure 2 clicks';
    }
    return;
  }
...
function toggleMeasure() {
  state.measureOn = !state.measureOn;
  state.measurePts = [];
  const btn = document.getElementById('btn-measure');
  const out = document.getElementById('measure-out');
  if (btn) btn.textContent = state.measureOn ? 'Measuring…' : 'Measure 2 clicks';
  if (out) out.textContent = state.measureOn ? 'click 1st point' : '—';
  if (state.measureOn) setStatus('Measure: click two points on the model');
}
...
function clipGeometryKeepMin(geometry, axis, plane) {
  ...
  function interp(i, j, ti, tj) {
    const t = (plane - ti) / (tj - ti);
    return [
      lerp(pos.getX(i), pos.getX(j), t),
      lerp(pos.getY(i), pos.getY(j), t),
      lerp(pos.getZ(i), pos.getZ(j), t)
    ];
  }
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    const c0 = axisCoord(axis, v0[0], v0[1], v0[2]);
    const c1 = axisCoord(axis, v1[0], v1[1], v1[2]);
    const c2 = axisCoord(axis, v2[0], v2[1], v2[2]);
    const k0 = keep(c0);
    const k1 = keep(c1);
    const k2 = keep(c2);
    const nKeep = (k0 ? 1 : 0) + (k1 ? 1 : 0) + (k2 ? 1 : 0);
    if (nKeep === 3) {
      pushTri(v0, v1, v2);
    } else if (nKeep === 0) {
      continue;
    } else if (nKeep === 1) {
      const verts = [[v0, c0, k0, i0], [v1, c1, k1, i1], [v2, c2, k2, i2]];
      const inside = verts.find(v => v[2]);
      const outs = verts.filter(v => !v[2]);
      const pA = interp(inside[3], outs[0][3], inside[1], outs[0][1]);
      const pB = interp(inside[3], outs[1][3], inside[1], outs[1][1]);
      pushTri(inside[0], pA, pB);
      edges.push([pA, pB]);
    } else {
      const verts = [[v0, c0, k0, i0], [v1, c1, k1, i1], [v2, c2, k2, i2]];
      const ins = verts.filter(v => v[2]);
      const outv = verts.find(v => !v[2]);
      const pA = interp(ins[0][3], outv[3], ins[0][1], outv[1]);
      const pB = interp(ins[1][3], outv[3], ins[1][1], outv[1]);
      pushTri(ins[0][0], ins[1][0], pA);
      pushTri(ins[1][0], pB, pA);
      edges.push([pA, pB]);
    }
  }
...
function cutActiveModel() {
  ...
  const cutMm = Number(document.getElementById('cut-mm').value);
  if (!(cutMm > 0.5)) {
    setStatus('Cut distance must be > 0.5 mm', true);
    return;
  }
  ...
  if (cutMm >= span - 0.5) {
    setStatus('Cut is past the far end (' + span.toFixed(1) + ' mm)', true);
    return;
  }
  const plane = min + cutMm;
  const geo = clipGeometryKeepMin(m.geometry, axis, plane);
  if (!geo) {
    setStatus('Cut produced an empty mesh — try another axis or mm', true);
    return;
  }
  addModel(m.name + '-cut' + Math.round(cutMm), geo);
  setStatus('Cut kept ' + cutMm.toFixed(1) + ' mm on ' + axis + '. Download that new model.');
}
...
function exportActiveModel() {
  const m = state.models.length ? state.models[state.models.length - 1] : null;
  if (!m) {
...
function setupUI() {
  ...
  if (btnMeasure) btnMeasure.addEventListener('click', toggleMeasure);
