...
function cutTFromPointer() {
  ...
  // Same bbox basis as getCutPlaneLocal so drag == split
  ...
  if (span < 1e-6) return;
  const meshX = state.previewMesh.position.x;
  const meshZ = state.previewMesh.position.z;
  const worldMin = axis === 'x' ? bbox.min.x + meshX : bbox.min.z + meshZ;
  const along = axis === 'x' ? hit.x : hit.z;
  const raw = Math.min(0.98, Math.max(0.02, (along - worldMin) / span));
...
function snapCutT(t, span) {
  if (!(span > 0)) return t;
  const mm = t * span;
  const snapped = Math.round(mm * 2) / 2; // 0.5 mm
  return Math.min(0.98, Math.max(0.02, snapped / span));
}

/** cutT is the only source of truth. Helper follows cutT (never the reverse). */
function getCutPlaneLocal(model) {
  ...
  const t = Math.min(0.98, Math.max(0.02, state.cutT));
  const origin = axis === 'x' ? bbox.min.x : bbox.min.z;
  const plane = origin + t * span;
  return { axis, plane, bbox, span, t };
}
...
function syncCutUI() {
  ...
  if (readout) {
    readout.textContent =
      'RED LINE @ ' + mm.toFixed(1) + ' mm  →  keep ' + mm.toFixed(1) + ' mm  |  ' +
      (span - mm).toFixed(1) + ' mm  (' + axis.toUpperCase() + ')';
  }
}
...
function openCutter() {
  ...
  restoreCamera(cam);
  updateCutterUI();
  setStatus('Cutter open — drag red plane, then Split. Length axis free in dropdown.');
}
...
function buildCutHelper() {
  ...
  // Solid-looking red plane (easier to see through the part)
...
function capFromEdges(edges, axis, plane, keepMin) {
  // Solid end plug: fill OUTER cross-section (convex hull of all cut points)
  // so hollow shells get a closed face, not just thin wall rims.
  ...
  function to2(p) {
    return axis === 'x' ? [p[2], p[1]] : [p[0], p[1]];
  }
  function from2(u, v) {
    if (axis === 'x') return [plane, v, u];
    return [u, v, plane];
  }

  // Unique welded points on the plane
  const pts = new Map();
  edges.forEach(pair => {
    pair.forEach(p => {
      const k = keyOf(p);
  ...
  const all3 = Array.from(pts.values());
  if (all3.length < 3) return [];

  // --- Convex hull in 2D (Andrew's monotone chain) ---
  const pts2 = all3.map((p, i) => {
    const t = to2(p);
    return { u: t[0], v: t[1], i };
  });
  pts2.sort((a, b) => a.u === b.u ? a.v - b.v : a.u - b.u);
  ...
  for (let i = 0; i < pts2.length; i++) {
    while (lower.length >= 2 && cross(lower[lower.length - 2], lower[lower.length - 1], pts2[i]) <= 0) lower.pop();
    lower.push(pts2[i]);
  }
  const upper = [];
  for (let i = pts2.length - 1; i >= 0; i--) {
    while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], pts2[i]) <= 0) upper.pop();
    upper.push(pts2[i]);
  }
  upper.pop();
  lower.pop();
  ...
  // Desired outward normal along cut axis
  const desired = {
    x: axis === 'x' ? (keepMin ? -1 : 1) : 0,
    y: 0,
    z: axis === 'z' ? (keepMin ? -1 : 1) : 0
  };

  // Fan-triangulate hull from centroid (convex → safe)
  let cu = 0, cv = 0;
  hull.forEach(h => { cu += h.u; cv += h.v; });
  cu /= hull.length;
  cv /= hull.length;
  const c3 = from2(cu, cv);

  const out = [];
  function pushOriented(a, b, d) {
    const abx = b[0] - a[0], aby = b[1] - a[1], abz = b[2] - a[2];
    ...
    if (flip) out.push(a[0], a[1], a[2], d[0], d[1], d[2], b[0], b[1], b[2]);
    else out.push(a[0], a[1], a[2], b[0], b[1], b[2], d[0], d[1], d[2]);
  }

  for (let i = 0; i < hull.length; i++) {
    const b = all3[hull[i].i];
    const d = all3[hull[(i + 1) % hull.length].i];
    pushOriented(c3, b, d);
  }
...
function cutActiveModel() {
  ...
  // Make sure helper matches cutT before reading plane
  updateCutHelper();
  const info = getCutPlaneLocal(m);
  if (!info) {
  ...
  // Cut exactly where the red plane sits
  ...
  // Add halves only — never remove or replace the source model
  addModel(nameA, left, { keepSelection: true, silent: true });
  addModel(nameB, right, { keepSelection: true, silent: true });
  ...
  // Select the larger half so the viewport shows a real new piece (not the full bar)
  const modelA = state.models.find(x => x.id === newIds[0]) || state.models.find(x => x.name === nameA);
  const modelB = state.models.find(x => x.id === newIds[1]) || state.models.find(x => x.name === nameB);
  ...
  const loss = span - sum;
  setStatus(
    'Split @ ' + cutMm.toFixed(1) + ' mm → ' + nameA + ' (' + mL.along.toFixed(1) +
    ' mm) + ' + nameB + ' (' + mR.along.toFixed(1) + ' mm). ' +
    'Showing larger half. Original still in list.' +
    (Math.abs(loss) > 1.5 ? ' ⚠ length loss ' + loss.toFixed(1) + ' mm' : '')
  );
  } catch (err) {
    console.error(err);
