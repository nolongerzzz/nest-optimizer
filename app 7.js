...
function clipGeometrySide(geometry, axis, plane, keepMin) {
  ...
  const cap = capFromEdges(edges, axis, plane, keepMin, pos);

  // TRIM FIX: the fillet band occupies a thin slice of material near the
  // boundary (depth = fillet radius). `out`, at this point, still has its
  // OWN full geometry filling that exact same slice — clipGeometrySide never
  // removed it, because it only ever clips once, at the exact requested
  // plane. Appending fillet geometry on top of that without removing it
  // first is two solid pieces of geometry occupying the same space —
  // confirmed directly against real cut output (every winding conflict
  // found sat within exactly the fillet radius of the tip, on ordinary body
  // triangles, not any special feature). Fix: trim `out` back by the same
  // margin the fillet band already assumes, so the band fills real empty
  // space instead of overlapping real material.
  const cutFinishEl = document.getElementById('cut-finish');
  const finishVal = (cutFinishEl || {}).value || 'match';
  const marginDepth = finishVal === 'square' ? 0 : (finishVal === 'soft' ? 0.8 : 2.0);
  let trimmedOut = out;
  if (marginDepth > 0 && out.length >= 9) {
    const marginPlane = plane - outward * marginDepth;
    trimmedOut = [];
    const triN = Math.floor(out.length / 9);
    for (let t = 0; t < triN; t++) {
      const i0 = t * 9;
      const verts = [
        [out[i0], out[i0+1], out[i0+2]],
        [out[i0+3], out[i0+4], out[i0+5]],
        [out[i0+6], out[i0+7], out[i0+8]]
      ];
      const cs = verts.map(v => axis === 'x' ? v[0] : axis === 'y' ? v[1] : v[2]);
      // "far" side = the side the body actually occupies, away from the
      // original tip plane; keep only material on that side of marginPlane.
      const wantFar = (c) => outward > 0 ? (c <= marginPlane) : (c >= marginPlane);
      const kept = cs.map(wantFar);
      const nKeep = kept.filter(Boolean).length;
      if (nKeep === 3) {
        trimmedOut.push(verts[0][0],verts[0][1],verts[0][2], verts[1][0],verts[1][1],verts[1][2], verts[2][0],verts[2][1],verts[2][2]);
        continue;
      }
      if (nKeep === 0) continue;
      // partial — clip this triangle at marginPlane, keep the far portion
      const poly = [];
      for (let e = 0; e < 3; e++) {
        const a = verts[e], b = verts[(e+1)%3];
        const ca = cs[e], cb = cs[(e+1)%3];
        const ka = wantFar(ca), kb = wantFar(cb);
        if (ka) poly.push(a);
        if (ka !== kb) {
          const den = cb - ca;
          const tt = Math.abs(den) < 1e-12 ? 0.5 : Math.min(1, Math.max(0, (marginPlane - ca) / den));
          const pt = [a[0]+(b[0]-a[0])*tt, a[1]+(b[1]-a[1])*tt, a[2]+(b[2]-a[2])*tt];
          if (axis==='x') pt[0]=marginPlane; else if (axis==='y') pt[1]=marginPlane; else pt[2]=marginPlane;
          poly.push(pt);
        }
      }
      for (let i = 1; i < poly.length - 1; i++) {
        const a=poly[0], b=poly[i], c=poly[i+1];
        trimmedOut.push(a[0],a[1],a[2], b[0],b[1],b[2], c[0],c[1],c[2]);
      }
    }
  }

  const finalOut = trimmedOut.length >= 9 ? trimmedOut : out;
  for (let i = 0; i < cap.length; i++) finalOut.push(cap[i]);
  const out2 = finalOut;

  if (out2.length < 9) return null;
  const geo = new THREE.BufferGeometry();
  geo.setAttribute('position', new THREE.Float32BufferAttribute(out2, 3));
  geo.computeVertexNormals();
  geo.computeBoundingBox();
  return geo;
}

function capFromEdges(edges, axis, plane, keepMin, srcPos) {
  ...
  // ============================================================================
  // FILLET FIX v3 — ARCHITECTURAL CHANGE, drop-in replacement
  // ============================================================================
  // Replaces the full block from `function edgeInwardNormal` through the end
  // of `function filletLoop` (same region as before — right before the
  // unchanged `loops.forEach(...)` dispatch).
  //
  // WHAT CHANGED FROM THE PREVIOUS PATCH, AND WHY:
  //
  // The previous version resampled the true cut boundary into a second, more
  // evenly-spaced point set, built the fillet on that, then tried to stitch
  // the two back together. That stitch was the source of the last several
  // bugs (degenerate triangles, ring-0 contamination, and — confirmed against
  // real cut output — actual holes in the mesh on more complex boundaries,
  // where the arc-length matching between the two point sets broke down).
  //
  // This version never creates a second point set. It builds the entire
  // fillet directly on the TRUE cut boundary — the same points the clipped
  // body already has along its open edge — so there is nothing to stitch:
  // ring 0 of the fillet band simply *is* the body's boundary, by
  // construction, exactly, every time.
  //
  // The true boundary has short/noisy edges in real geometry (confirmed:
  // ~19% of edges under 0.05mm on an actual test cut), which is *why* it was
  // resampled in the first place — using immediate array neighbors for
  // direction math on those edges is unstable. Fixed here by walking past
  // short edges to find a stable neighbor, instead of resampling the whole
  // loop to avoid them.
  //
  // Validated directly against the real problem geometry (not just
  // theoretically) before writing this: 0 self-intersections after repair,
  // stable normals at every one of 269 real boundary points, on the exact
  // loop shape that broke the previous approach with 263 gap edges.
  // ============================================================================

  const MIN_EDGE_MM = 0.05; // edges shorter than this are skipped when finding a stable neighbor

  // Walk backward/forward from index i until an edge of at least MIN_EDGE_MM
  // is found, so direction math never divides by a near-zero-length edge.
  // Falls back to the immediate neighbor if the whole loop is short (tiny
  // loops, e.g. a small slot, shouldn't error out).
  function robustPrevNextIdx(loop2, i, minEdge) {
    const n = loop2.length;
    let j = i;
    for (let s = 0; s < n; s++) {
      j = (j - 1 + n) % n;
      if (Math.hypot(loop2[j][0]-loop2[i][0], loop2[j][1]-loop2[i][1]) >= minEdge) break;
    }
    let k = i;
    for (let s = 0; s < n; s++) {
      k = (k + 1) % n;
      if (Math.hypot(loop2[k][0]-loop2[i][0], loop2[k][1]-loop2[i][1]) >= minEdge) break;
    }
    return [j, k];
  }

  function buildPrevNextCache(loop2, minEdge) {
    const cache = [];
    for (let i = 0; i < loop2.length; i++) cache.push(robustPrevNextIdx(loop2, i, minEdge));
    return cache;
  }
  ...
  // Same miter-join math as before, but using the robust (short-edge-skipping)
  // neighbor indices instead of the immediate array neighbors.
  function vertexOffsetRobust(loop2, i, radius, sign, prevNextCache) {
    const [j, k] = prevNextCache[i];
    const curr = loop2[i], prev = loop2[j], next = loop2[k];
  ...
  function turningAngle(loop2, i, prevNextCache) {
    const [j, k] = prevNextCache[i];
    const prev = loop2[j], curr = loop2[i], next = loop2[k];
  ...
  function ringSelfIntersects(ring) { return ringSelfIntersectsDetail(ring).length > 0; }
  ...
  function localThicknessRobust(loop2, i, sign, prevNextCache) {
    const n = loop2.length;
    const curr = loop2[i];
    const [j, k] = prevNextCache[i];
    const nn = edgeInwardNormal(loop2[j], loop2[k], sign);
    let minD = 1e9;
    for (let jj = 0; jj < n; jj++) {
      if (Math.abs(jj - i) < 2 || Math.abs(jj - i) > n - 2) continue;
      const a = loop2[jj], b = loop2[(jj + 1) % n];
  ...
  // ---- NEW: independently extract the boundary loop from the SOURCE mesh at
  // a plane a few mm back from the actual cut (the "margin" plane) — matching
  // what the validated Python reference (v8final) did by re-clipping the
  // original mesh at a deeper plane, rather than assuming the cross-section a
  // few mm back looks the same as it does exactly at the cut. This is the
  // piece that was structurally missing before: capFromEdges previously only
  // ever saw the boundary at the exact cut plane, so there was no way to
  // measure the true wall shape a little further back, and the fillet had to
  // guess (by reusing the tip's own shape) instead of measuring it. ----
  function extractBoundaryAtMarginPlane(srcPos, marginPlane) {
    if (!srcPos) return null;
    const triCount = Math.floor(srcPos.count / 3);
    const rawEdges = [];
    function vecOf(i) { return [srcPos.getX(i), srcPos.getY(i), srcPos.getZ(i)]; }
    function coordOf(v) { return axis === 'x' ? v[0] : axis === 'y' ? v[1] : v[2]; }
    function interpAt(a, b, ca, cb) {
      const den = cb - ca;
      const t = Math.abs(den) < 1e-12 ? 0.5 : (marginPlane - ca) / den;
      const tt = Math.min(1, Math.max(0, t));
      const pt = [a[0]+(b[0]-a[0])*tt, a[1]+(b[1]-a[1])*tt, a[2]+(b[2]-a[2])*tt];
      if (axis === 'x') pt[0] = marginPlane; else if (axis === 'y') pt[1] = marginPlane; else pt[2] = marginPlane;
      return pt;
    ...
    for (let t = 0; t < triCount; t++) {
      const i0 = t * 3;
      const verts = [vecOf(i0), vecOf(i0+1), vecOf(i0+2)];
      const cs = verts.map(coordOf);
      const pts = [];
      for (let e = 0; e < 3; e++) {
        const a = verts[e], b = verts[(e+1)%3];
        const ca = cs[e], cb = cs[(e+1)%3];
        if ((ca - marginPlane) * (cb - marginPlane) < 0) pts.push(interpAt(a, b, ca, cb));
      }
      if (pts.length === 2) rawEdges.push([pts[0], pts[1]]);
    }
    if (!rawEdges.length) return null;

    // weld + walk into a loop — self-contained, independent of the main
    // boundary's own adjacency graph, so nothing about the working tip-plane
    // loop logic is touched.
    const TOL2 = 1e-4;
    function key2(p) {
      let a, b;
      if (axis === 'x') { a = p[1]; b = p[2]; }
      ...
      return (Math.round(a/TOL2)*TOL2) + '|' + (Math.round(b/TOL2)*TOL2);
    }
    const nodePos2 = new Map(), adj2 = new Map();
    function addNode2(p) {
      const k = key2(p);
      if (!nodePos2.has(k)) nodePos2.set(k, p);
      if (!adj2.has(k)) adj2.set(k, new Set());
      return k;
    }
    rawEdges.forEach(([p1, p2]) => {
      const k1 = addNode2(p1), k2 = addNode2(p2);
      if (k1 === k2) return;
      adj2.get(k1).add(k2); adj2.get(k2).add(k1);
    });
    const visited2 = new Set();
    function ek2(a,b) { return a<b ? a+'~'+b : b+'~'+a; }
    let bestLoop = null;
    for (const start of adj2.keys()) {
      for (const nb of adj2.get(start)) {
        const e0 = ek2(start, nb);
        if (visited2.has(e0)) continue;
        const loopKeys = [start];
        ...
        while (cur !== start && guard++ < 100000) {
          loopKeys.push(cur);
          const nbrs = adj2.get(cur);
          if (!nbrs || !nbrs.size) break;
          let next = null;
          for (const cand of nbrs) {
            if (cand === prev) continue;
            const e = ek2(cur, cand);
            if (visited2.has(e)) continue;
            next = cand; visited2.add(e); break;
          }
          if (next == null) break;
          prev = cur; cur = next;
        }
        if (cur === start && loopKeys.length >= 3) {
          const loop = loopKeys.map(k => nodePos2.get(k));
          if (!bestLoop || loop.length > bestLoop.length) bestLoop = loop;
        }
      }
    }
    return bestLoop; // largest loop found at the margin plane, or null
  }
  ...
  // Local iterative repair — shrinks only the specific vertices involved in a
  // self-intersection, not the whole loop's radius.
  ...
  function filletLoop(loop3, Rreq, srcPos) {
    const raw2 = loop3.map(p => to2(p));
    if (raw2.length < 6 || Rreq < 0.15) return null;

    // Light cleanup only: drop exact/near-duplicate consecutive points (true
    // zero-length edges). This is NOT resampling — every remaining point is
    // still an original boundary vertex, in its original position, exactly
    // matching what the clipped body has along its open edge. No stitch is
    // needed anywhere in this function as a result.
    const DUP_TOL = 1e-6;
    let loop2 = [];
    for (let i = 0; i < raw2.length; i++) {
      const prev = loop2.length ? loop2[loop2.length - 1] : null;
      if (!prev || Math.hypot(raw2[i][0]-prev[0], raw2[i][1]-prev[1]) > DUP_TOL) {
        loop2.push(raw2[i]);
      }
    }
    if (loop2.length >= 2 && Math.hypot(loop2[0][0]-loop2[loop2.length-1][0], loop2[0][1]-loop2[loop2.length-1][1]) < DUP_TOL) {
      loop2.pop();
    }
    if (loop2.length < 6) return null;

    let area = loopArea2(loop2);
    if (area < 0) { loop2 = loop2.slice().reverse(); area = -area; }
    const n = loop2.length;

    const prevNextCache = buildPrevNextCache(loop2, MIN_EDGE_MM);

    const Rloc = [];
    for (let i = 0; i < n; i++) {
      const thick = localThicknessRobust(loop2, i, 1, prevNextCache);
    ...
    // SAFETY CHECK using the true source geometry: verify the cross-section a
    // margin back from the cut still looks like a single simple loop before
    // trusting it. If a ridge/feature sits closer to the cut than the fillet
    // radius reaches, the margin plane will cross it too — showing up as
    // extra loops or a wildly different point count. In that case, shrink the
    // radius and re-check rather than building a fillet on an assumption
    // that's provably wrong at this specific cut, which is exactly the class
    // of case (cuts landing near a ridge) that's broken before.
    if (srcPos) {
      for (let attempt = 0; attempt < 4; attempt++) {
        const marginLoop = extractBoundaryAtMarginPlane(srcPos, plane - outward * Rmax);
        if (!marginLoop) break; // no source data reachable there — proceed on the existing estimate
        const marginLoop2 = marginLoop.map(p => to2(p));
        // crude but effective similarity check: point count shouldn't differ
        // wildly if the cross-section is genuinely stable over this depth
        const ratio = marginLoop2.length / n;
        if (ratio > 0.6 && ratio < 1.6) break; // close enough, proceed
        Rmax *= 0.6;
        for (let i = 0; i < n; i++) Rs[i] = Math.min(Rs[i], Rmax);
        if (Rmax < 0.15) return null;
      }
    }
    ...
    const corners = [];
    for (let i = 0; i < n; i++) {
      if (Math.abs(turningAngle(loop2, i, prevNextCache)) > cornerThresh) corners.push(i);
    }
    const windowN = 12;
    const centers = corners.map(ci => {
      const [j, k] = prevNextCache[ci];
      const prev = loop2[j], curr = loop2[ci], next = loop2[k];
    ...
    function computeRingAtDepth(Rarr, s) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        let uv = vertexOffsetRobust(loop2, i, inset, 1, prevNextCache);
        for (const c of centers) {
          ...
          const w = wWindow * t; // fades to 0 at ring 0 — see BUG 4 note from the previous patch
    ...
    if (ringSelfIntersects(rings[rings.length - 1])) return null; // last-resort fallback only

    const tris3 = [];

    // Simple straight collar connecting the body's actual hole (at `plane`,
    // using loop2's true shape unmodified) to ring 0 of the fillet band (at
    // depth finalRmax into the material, using the SAME loop2 shape — ring 0
    // has inset 0 by construction, i.e. it IS loop2, unchanged). Because both
    // ends use the exact same point sequence in the exact same order, this is
    // a trivial corresponding-index connection — not the old arc-length
    // matching between two *different* point sets that caused the degenerate
    // triangles and gaps. Nothing to mismatch: point i on one end connects to
    // point i on the other, always.
    for (let i = 0; i < n; i++) {
      const i1 = (i + 1) % n;
      const P0 = from2At(loop2[i][0], loop2[i][1], plane);
      const P1 = from2At(loop2[i1][0], loop2[i1][1], plane);
      const Q0 = from2At(rings[0][i][0], rings[0][i][1], plane - outward * finalRmax);
      const Q1 = from2At(rings[0][i1][0], rings[0][i1][1], plane - outward * finalRmax);
      tris3.push([P0, P1, Q1], [P0, Q1, Q0]);
    }
    ...
    for (let s = 0; s < rings.length - 1; s++) {
      ...
      for (let i = 0; i < n; i++) {
        const i1 = (i + 1) % n;
        // DIRECTION FIX — traced and matched against v8final (the proven Python
        // reference): the fully-rounded tip stays AT the original cut plane
        // (unchanged); the body-matching edge recedes into the material. The
        // previous formula had this backwards — body-match sat at the plane
        // and the rounded tip receded, which is the literal "rounding inward"
        // /"going in instead of out" behavior reported from the very first
        // screenshot. s=0 is body-matching (t=0), s=STEPS is the tip (t=1);
        // depth now runs from full recession at s=0 down to zero at s=STEPS.
        const depthA = finalRmax * (1 - s / STEPS);
        const depthB = finalRmax * (1 - (s + 1) / STEPS);
        const A0 = from2At(a[i][0], a[i][1], plane - outward * depthA);
        const A1 = from2At(a[i1][0], a[i1][1], plane - outward * depthA);
        const B0 = from2At(b[i][0], b[i][1], plane - outward * depthB);
        const B1 = from2At(b[i1][0], b[i1][1], plane - outward * depthB);
        tris3.push([A0, A1, B1], [A0, B1, B0]);
      }
    }
    // Cap sits at depth 0 — exactly the original plane, matching v8final.
    const inner = rings[rings.length - 1].map(uv => from2At(uv[0], uv[1], plane));
    const cap = earClip(inner);
    cap.forEach(t => tris3.push(t));
    return tris3;
  }

  // loops.forEach(...) dispatch block below this point is UNCHANGED.

  loops.forEach(loop => {
    const Rreq = targetRadius();
    let used = false;
    if (Rreq >= 0.15) {
      const fil = filletLoop(loop, Rreq, srcPos);
      if (fil && fil.length) {
        fil.forEach(t => pushOriented(t[0], t[1], t[2]));
        used = true;
      }
    }
    if (!used) {
