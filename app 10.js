...
// ============================================================================
// FILLET ENGINE — built and verified fresh this session against real geometry.
// Every function here was tested against actual STL data before integration,
// not assumed correct. See engine PROGRESS.md for the full build log.
// Single-loop cross-sections only (multi-loop/slots deliberately deferred —
// see PROGRESS.md). Ridge-adjacent anomaly at specific cut locations also
// deferred, deliberately, per direct instruction. Duplicate small helper
// functions across source files (each file was built to be independently
// testable) are deduplicated below, verified identical before removal.
// ============================================================================
...
function earClip2D(poly2d) {
  ...
  while (work.length > 3 && guard++ < 200000) {
    ...
    if (!found) break; // shouldn't happen for a simple polygon; caller should check
  }
...
// (signedArea2D already defined above, identical in thickness.mjs, skipped)
...
// (edgeNormal already defined above, identical in corners.mjs, skipped)
/**
...
// ---- from band.mjs ----
// (edgeNormal already defined above, identical in band.mjs, skipped)
// (robustPrevNext already defined above, identical in band.mjs, skipped)
...
// ---- from fillet.mjs ----
// (to2D already defined above, identical in fillet.mjs, skipped)
// (from2D already defined above, identical in fillet.mjs, skipped)
/**
...
function cutAndFillet(tris, axisIdx, planeVal, keepMin, requestedR) {
  ...
  const tipLoop3d = tipLoops[0]; // TODO: handle multi-loop cross-sections (slots) — next stage
  const poly2d = tipLoop3d.map(p => to2D(p, axisIdx));

  if (requestedR < 0.15) {
    // square finish — no fillet, just flat cap
    const { kept } = clipTrianglesAtPlane(tris, axisIdx, planeVal, keepMin);
    const cap = flatCapLoop(tipLoop3d, axisIdx, planeVal, keepMin);
    return kept.concat(cap);
  }
  ...
  const marginLoop2d = marginLoops[0].map(p => to2D(p, axisIdx));
