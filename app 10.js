...
// ===================== Orientation =====================
// mode: 'density' = max models per plate | 'supports' = fewer overhangs
function scoreOrientation(geometry, mode = 'density') {
  ...
  let score;
  if (mode === 'density') {
    score = footprint * 1.0 + maxSide * 10 + height * 0.2;
  } else {
    score = height * 2.2 - footprint * 0.008 + maxSide * 0.3;
  }
  return { score, size, footprint, height, maxSide };
}
...
function packModels(instances, plate, gap, allowRotate) {
  ...
  for (const inst of sorted) {
    ...
    if (!best) {
      // Still place it so the user can rotate/nudge manually
      ...
      placed.push({
        ...
        rotated: false,
        overflow: true,
        // slight visual offset so stacked overflows are selectable
...
function runOptimize() {
  ...
  try {
    ...
    // Build instances for a given orient mode
    function buildInstances(mode) {
      ...
      for (const model of state.models) {
        ...
        let size = model.size;
        if (mode) {
          const best = autoOrient(model.geometry, mode);
          geo = best.geometry;
          size = { x: best.size.x, y: best.size.y, z: best.size.z };
        } else {
          geo = model.geometry.clone();
          geo.center();
        }
    ...
    // Try density-first (max count). Also try supports mode if checkbox on.
    // Keep whichever fits more models on the plate.
    const candidates = [];
    ...
    candidates.push({ mode: null, instances: buildInstances(null) }); // original

    let bestPlaced = null;
    let bestFitted = -1;
    let bestMode = 'density';

    for (const c of candidates) {
      const placed = packModels(c.instances, plate, gap, allowRotate);
      const fitted = placed.filter(p => !p.overflow).length;
      if (fitted > bestFitted) {
        bestFitted = fitted;
        bestPlaced = placed;
        bestMode = c.mode || 'original';
      }
    }
    ...
    placed.forEach((p, idx) => {
      ...
      const hue = p.overflow ? 0.05 : (0.55 + (idx % 8) * 0.03); // orange-ish if needs manual place
      mesh.material.color.setHSL(hue, 0.7, 0.55);
      mesh.userData.placedIndex = idx;
      p.mesh = mesh;
      p.rotY = p.rotated ? Math.PI / 2 : 0;
      state.modelGroup.add(mesh);
    });
    ...
    const fitted = placed.length - overflowCount;
    if (overflowCount > 0) {
...
function selectPlaced(idx) {
  ...
  if (p && p.mesh) {
    p.mesh.material.emissive = new THREE.Color(0x224466);
...
function updateAdjustUI() {
  ...
  ['btn-rot-left', 'btn-rot-right', 'btn-flip', 'btn-nudge-left', 'btn-nudge-right', 'btn-nudge-fwd', 'btn-nudge-back']
    .forEach(id => {
...
function rotateSelected(dir) {
  ...
  p.rotated = Math.abs(Math.sin(p.rotY)) > 0.5; // roughly 90° or 270°
...
function flipSelected() {
  ...
  p.flipX = !p.flipX;
  p.mesh.rotation.x = p.flipX ? Math.PI : 0;
...
function buildCombinedGeometry() {
  ...
  state.placed.forEach(p => {
    ...
    if (p.flipX) geo.rotateX(Math.PI);
